...
Mid-wave instrument note: after both arm summaries were committed, main landed a fail-closed guard refusing local research reads of a stale `massive_stock_day` mirror (#5319 — the store-refresh workstream's first landing). The W2 harness now threads that guard's explicit override, declaring the frozen 2026-07-02 vintage deliberate (same-tape comparability, prereg §2). Both arm runs predate the merge; no result was computed under the new code path.
