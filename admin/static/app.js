...
RENDER.mastermind_ai = async () => {
  ...
  if (runBtn) runBtn.onclick = async () => {
    ...
    const r = await post("/api/mastermind_ai/run", {});   /* bot replies with the loop-log row */
    runBtn.disabled = false; runBtn.textContent = "▶ Run cycle now";
    if (r && !r.error && r.ok !== false) {
      toast(r.summary ? `Cycle done — ${orchTrunc(r.summary, 140)}` : "Cycle done");
      if (CURRENT === "mastermind_ai") RENDER.mastermind_ai();   /* re-render only after the run resolved */
    }
    else toast((r && (r.error || r.detail || r.skipped)) || "run failed", true);
  };
