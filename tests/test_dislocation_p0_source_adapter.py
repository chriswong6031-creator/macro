...
from engine.fundamental_forensics.sec_document_spine import build_filing_manifests, with_document_retrievals
...
def _stored_manifest(root: Path, slot: int, *, form: str = "8-K", accepted: str | None = ACCEPTED) -> tuple[dict, str]:
    ...
    receipt = persist_archive_document(root, primary, f"source-{slot}".encode(), retrieved_at=RECORDED)
...
def _panel(root: Path) -> tuple[list[CanonicalSpineRef], list[dict]]:
    ...
    for slot in range(1, 21):
        ...
        refs.append(CanonicalSpineRef(slot, manifest["issuer"]["cik"], manifest["filing"]["accession"], manifest["filing"]["base_form"], manifest["clocks"]["filed_on"], key))
...
def test_returns_twenty_verbatim_owner_packets_and_never_writes(tmp_path: Path) -> None:
    ...
    assert first.primary_document == owner["documents"][0]
    assert first.source_bytes == b"source-1"


def test_atomically_refuses_absent_historical_owner_reference(tmp_path: Path) -> None:
    refs, _ = _panel(tmp_path)
    refs[7] = CanonicalSpineRef(8, refs[7].cik, refs[7].accession, refs[7].expected_base_form, refs[7].expected_filed_on, None)
    result = read_exact_p0_source_packets(archive_root=tmp_path, refs=refs)
...
def test_crosswire_tamper_and_amendment_are_typed_gaps(tmp_path: Path) -> None:
    refs, _ = _panel(tmp_path)
    refs[0] = CanonicalSpineRef(1, "9999999999", refs[0].accession, refs[0].expected_base_form, refs[0].expected_filed_on, refs[0].manifest_storage_key)
    result = read_exact_p0_source_packets(archive_root=tmp_path, refs=refs)
    ...
    refs[0] = CanonicalSpineRef(1, amended["issuer"]["cik"], amended["filing"]["accession"], amended["filing"]["base_form"], amended["clocks"]["filed_on"], key)
...
def test_cardinality_and_exact_acceptance_fail_closed(tmp_path: Path) -> None:
    ...
    refs[0] = CanonicalSpineRef(1, missing_clock["issuer"]["cik"], missing_clock["filing"]["accession"], missing_clock["filing"]["base_form"], missing_clock["clocks"]["filed_on"], key)
...
def test_frozen_base_form_and_filed_on_must_match_owner(tmp_path: Path) -> None:
    refs, _ = _panel(tmp_path)
    refs[0] = CanonicalSpineRef(1, refs[0].cik, refs[0].accession, "6-K", refs[0].expected_filed_on, refs[0].manifest_storage_key)
    result = read_exact_p0_source_packets(archive_root=tmp_path, refs=refs)
    assert result.packets == () and result.gaps[0].code == "OWNER_IDENTITY_MISMATCH"

    refs, _ = _panel(tmp_path / "filed")
    refs[0] = CanonicalSpineRef(1, refs[0].cik, refs[0].accession, refs[0].expected_base_form, "2026-08-19", refs[0].manifest_storage_key)
