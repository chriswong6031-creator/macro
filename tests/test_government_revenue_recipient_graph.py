...
def test_canonical_reviewed_graph_is_strict_and_keeps_absent_event_coverage_empty():
    ...
    assert graph["graph_id"] == "recipient-graph:reviewed:2026-08-11:defense20-v1"
    assert len(graph["evidence"]) == 305
    assert len(graph["companies"]) == 20
    assert len(graph["legal_entities"]) == 122
    assert len(graph["identifiers"]) == 265
    assert len(graph["ownership_edges"]) == 122
