"""Adversarial tests for Prophet Lab temporal correctness plus MAS-123 Cell G VOI law.

The original section pins ``engine.prophet_lab.timeparse``.  The Cell G section is
intentionally colocated here because this file is already owned by the canonical
``prophet-lab`` semantic CI job; this avoids creating a parallel CI workflow while
ensuring the new measurement/refusal laws execute on every relevant PR.
"""
...
def test_parse_instant_before_across_offset_forms() -> None:
    reference = parse_instant("2026-08-19T10:00:00Z")
    for form in [
        ...
        assert parsed is not None and parsed < reference
...
def test_regression_lexicographic_compare_would_have_picked_the_wrong_earliest() -> None:
    a = "2026-08-19T10:00:00Z"
    b = "2026-08-19T09:00:00-04:00"
    assert b < a
    assert parse_instant(a) < parse_instant(b)
...
# ---------------------------------------------------------------------------
from engine.prophet_voi import (  # noqa: E402
    DESCRIPTIVE_ONLY,
    HOLD_INTEGRITY,
    MEASURED,
    PROTECTED_OUTCOME,
...
from engine.prophet_voi_eawc import (  # noqa: E402
...
from scripts import prophet_flagship_voi_report as voi_report  # noqa: E402
