...
RESEARCH_FEED_PLIST = RESEARCH_FEED_ROOT / "ops" / "launchd" / "com.mastermindx.research-feed.plist"
RESEARCH_FEED_CANONICAL_DB = "/Volumes/STORAGE/MastermindX/marketdesk/db/marketdesk.sqlite"
RESEARCH_FEED_CANONICAL_REPO = "mastermindx-market-intelligence/macro"
RESEARCH_FEED_LEGACY_REPO = "chriswong6031-creator/macro"
RESEARCH_FEED_LEGACY_DB_SUFFIX = "mastermind-research/marketdesk_paper_extractor/db/marketdesk.sqlite"


def _rf_write_db(path: Path, timestamps: list[str]) -> None:
...
def _rf_write_executable(path: Path, content: str) -> None:
...
def _rf_run_feed(
    ...
    *,
    ...
    legacy_db = tmp_path / "home" / RESEARCH_FEED_LEGACY_DB_SUFFIX
    ...
    if db_override != "missing":
        if valid_schema:
            _rf_write_db(external_db, timestamps)
        else:
    ...
    if legacy_timestamps is not None or db_override == "legacy":
        _rf_write_db(legacy_db, legacy_timestamps if legacy_timestamps is not None else timestamps)
    if db_override == "legacy_data":
        _rf_write_db(legacy_data_db, timestamps)
...
""",
    ...
    env.update(
        {
            ...
            "RESEARCH_FEED_REPO": RESEARCH_FEED_CANONICAL_REPO,
    ...
    result = subprocess.run(
        ["/bin/bash", str(RESEARCH_FEED_RUNNER)],
        cwd=RESEARCH_FEED_ROOT,
        env=env,
        text=True,
        capture_output=True,
        check=False,
...
def _rf_commands(paths: dict[str, Path]) -> list[str]:
...
def test_tracked_runner_and_plist_are_parseable_and_pin_the_existing_service() -> None:
    assert RESEARCH_FEED_RUNNER.is_file()
    assert RESEARCH_FEED_PLIST.is_file()
    parsed = subprocess.run(
        ["/bin/bash", "-n", str(RESEARCH_FEED_RUNNER)],
        text=True,
        capture_output=True,
        check=False,
    )
    ...
    with RESEARCH_FEED_PLIST.open("rb") as handle:
...
def test_runner_defaults_to_the_producers_canonical_storage_db() -> None:
    source = RESEARCH_FEED_RUNNER.read_text(encoding="utf-8")
    assert RESEARCH_FEED_CANONICAL_DB in source
    assert RESEARCH_FEED_CANONICAL_REPO in source
    assert RESEARCH_FEED_LEGACY_REPO not in source
...
def test_external_db_wins_when_a_stale_legacy_db_is_present(tmp_path: Path) -> None:
    ...
    assert sum(command.startswith("workflow run") for command in _rf_commands(paths)) == 1
...
def test_missing_watermark_adopts_current_state_without_dispatch(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stderr
    assert paths["watermark"].read_text(encoding="utf-8").strip() == latest
    assert not any(command.startswith("workflow run") for command in _rf_commands(paths))
...
def test_no_new_rows_does_not_query_or_dispatch_github(tmp_path: Path) -> None:
    latest = "2026-09-06T14:00:00+00:00"
    result, paths = _rf_run_feed(tmp_path, timestamps=[latest], watermark=latest)
    assert result.returncode == 0, result.stderr
    assert _rf_commands(paths) == []
    assert paths["watermark"].read_text(encoding="utf-8").strip() == latest
...
def test_new_row_dispatches_once_and_advances_watermark_only_after_success(tmp_path: Path) -> None:
    ...
    assert f"--repo {RESEARCH_FEED_CANONICAL_REPO}" in dispatch
