...
def test_known_gaps_are_suppressed_but_new_breakage_is_not(tmp_path):
    site = _site(tmp_path)
    pinned = sorted(_KNOWN_GAPS)[0]
    (site / "stocks" / "A.html").write_text(
        f'<a href="/{pinned}">pinned</a><a href="/stocks/BRANDNEW.html">new</a>',
        encoding="utf-8")
    assert find_dangling(site) == {"stocks/BRANDNEW.html": ["stocks/A.html"]}


def test_every_pinned_gap_is_site_relative_and_unstamped(tmp_path):
