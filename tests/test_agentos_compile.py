...
``tests.agentos_compile_legacy_cases`` module.  The new projector cases live in the
non-collectable ``tests.linear_portfolio_plan_cases`` module.  This explicit aggregator
keeps one CI-owned pytest suite at the path already named by the always-on
``self-mod-fence`` job; it does not hide or waive either test family.
"""
