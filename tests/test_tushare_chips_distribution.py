...
def vendor_frame(percents=(25.0, 25.0, 25.0, 25.0), prices=("10", "11", "12", "13"),
                 trade_date: str = COMPACT) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {"ts_code": VENDOR_CODE, "trade_date": trade_date, "price": float(p),
             "percent": pct}
...
def test_partition_round_trips_with_exact_decimal_prices(tmp_path):
    result = chips.collect(
        TICKER, TRADE_DATE, output_root=tmp_path,
        query_fn=query_returning(vendor_frame()), now=FIXED_NOW,
    )
    ...
    assert [r["price"] for r in rows] == [
        Decimal("10.0000"), Decimal("11.0000"), Decimal("12.0000"), Decimal("13.0000")
    ]
...
def test_receipt_pins_contract_authority_and_recomputes(tmp_path):
    chips.collect(TICKER, TRADE_DATE, output_root=tmp_path,
                  query_fn=query_returning(vendor_frame()), now=FIXED_NOW)
    path = tmp_path / f"by_trade_date={TRADE_DATE}" / f"by_ticker={TICKER}" / "receipt.json"
    ...
    # Licensing is SETTLED by the operator's statement, so the receipt cites it and carries
    # no purchase/payment/license nonclaims; only epistemic ones survive.
    license_statement = receipt["license_statement"]
    assert license_statement["mode"] == "operator_attestation_verified"
    assert license_statement["statement_document_sha256"] == (
        "ad48d044c8a763435f232fa81f44908817d04b28eb9bc9e6175e2c06fd6265fb"
    )
    assert license_statement["statement_document"].endswith(
        "TUSHARE_ADDONS_COLLECTOR_FOUNDATION_2026-08-09.md"
    )
    assert "2035" in license_statement["statement_summary"]
    ...
    for retired in ("not_proof_of_purchase", "not_proof_of_payment",
                    "not_proof_of_license", "no_team_sharing"):
    ...
    assert data["percent_mass_observation"] == "consistent_with_documented_percentage_points"
...
def test_recollecting_identical_rows_is_a_no_op(tmp_path):
    first = chips.collect(TICKER, TRADE_DATE, output_root=tmp_path,
                          query_fn=query_returning(vendor_frame()), now=FIXED_NOW)
    second = chips.collect(TICKER, TRADE_DATE, output_root=tmp_path,
                           query_fn=query_returning(vendor_frame()), now=FIXED_NOW)
...
def test_keep_first_refuses_a_revised_response(tmp_path):
    ...
    with pytest.raises(chips.CollectorIntegrityError, match="keep-first"):
        chips.collect(TICKER, TRADE_DATE, output_root=tmp_path,
                      query_fn=query_returning(revised), now=FIXED_NOW)
...
def test_fraction_like_percent_mass_is_a_loud_stop(tmp_path):
    ...
    with pytest.raises(chips.CollectorIntegrityError, match="fraction-like"):
        chips.collect(TICKER, TRADE_DATE, output_root=tmp_path,
                      query_fn=query_returning(fraction_like), now=FIXED_NOW)
...
def test_indeterminate_mass_is_recorded_not_rejected(tmp_path):
    ...
    result = chips.collect(TICKER, TRADE_DATE, output_root=tmp_path,
                           query_fn=query_returning(partial), now=FIXED_NOW)
    assert result.status == "written"
...
def test_price_beyond_the_pinned_decimal_scale_raises(tmp_path):
    ...
    with pytest.raises(chips.CollectorIntegrityError, match="decimal scale"):
        chips.collect(TICKER, TRADE_DATE, output_root=tmp_path,
                      query_fn=query_returning(frame), now=FIXED_NOW)
...
def test_row_escaping_the_requested_session_raises(tmp_path):
    ...
    with pytest.raises(chips.CollectorIntegrityError, match="escaped the requested session"):
        chips.collect(TICKER, TRADE_DATE, output_root=tmp_path,
                      query_fn=query_returning(frame), now=FIXED_NOW)
...
def test_empty_response_is_held_not_written(tmp_path):
    ...
    with pytest.raises(chips.CollectionHeld) as excinfo:
        chips.collect(TICKER, TRADE_DATE, output_root=tmp_path,
                      query_fn=query_returning(empty), now=FIXED_NOW)
...
def test_governor_stays_under_the_shared_premium_budget():
    ...
    assert chips._MIN_CALL_INTERVAL_S == pytest.approx(60.0 / chips.MAX_CALLS_PER_MINUTE)
...
def test_reference_dependent_features_are_null_without_a_reference():
    ...
    assert chips.winner_shares(levels, None) == {
        "winner_share": None, "loser_share": None, "at_cost_share": None
    }
...
def ranged_frame(dates=RANGE_DATES) -> pd.DataFrame:
    ...
    for d in dates:
        for price, pct in zip((10.0, 11.0, 12.0, 13.0), (10.0, 60.0, 20.0, 10.0)):
            rows.append({"ts_code": VENDOR_CODE, "trade_date": d,
                         "price": price, "percent": pct})
...
def test_history_accrual_writes_derived_features_not_raw_levels(tmp_path, monkeypatch):
    ...
    assert {"chip_entropy_norm", "chip_peak_price", "chip_avg_cost",
            "chip_level_count", "chip_winner_share", "chip_conc_5pct",
            "chip_conc_10pct"} <= set(stored.columns)
    row = stored.iloc[0]
...
def test_history_accrual_is_bounded_by_the_call_cap(tmp_path, monkeypatch):
    calls: list[dict] = []
    monkeypatch.setattr(hist.tc, "query",
                        lambda api, **kw: (calls.append(kw), ranged_frame())[1])
    monkeypatch.setattr(hist, "_close_panel", lambda: None)
    panel = {TICKER, "000001.SZ", "600000.SS"}

    hist._accrue_chips_distribution(tmp_path / "h.parquet", panel, RANGE_DATES, max_calls=2)
    assert len(calls) == 2
