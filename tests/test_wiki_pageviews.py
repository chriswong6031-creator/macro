...
def _render_achip(r):
    ...
    pre = src[: src.index("<!DOCTYPE")]                    # t() + help() + achip() macros
    env = Environment(loader=FileSystemLoader(str(TEMPLATES)), autoescape=True)
    env.globals.update(td=i18n.td, tr=i18n.tr)
    return env.from_string(pre + "{{ achip(r) }}").render(r=r)


def test_chip_bilingual_us():
    html = _render_achip({"ticker": "AAPL", "attn_z": 2.5})
    ...
    assert "international attention only" not in html      # US ticker -> no CN/HK caveat


def test_chip_cnhk_caveat():
    html = _render_achip({"ticker": "600519.SS", "attn_z": 2.5})
    assert "international attention only" in html and "仅衡量境外关注度" in html


def test_chip_absent_below_threshold():
    assert _render_achip({"ticker": "AAPL", "attn_z": 1.2}).strip() == ""
    assert _render_achip({"ticker": "AAPL", "attn_z": None}).strip() == ""
