...
def test_engine_render_outlier_is_partitioned_without_coverage_loss() -> None:
    ...
    # The manifest grows whenever main wires another suite, so its total is not
    # a stable property of this split. Pin the scheduling property instead: the
    # deterministic greedy partition must remain within two units of the
    # arithmetic lower bound while the former 1,036-unit singleton stays gone.
    lower_bound = (sum(pack_weights) + 11) // 12
    assert max(pack_weights) <= lower_bound + 2
...
def _document_plan_sha(document: dict[str, object]) -> str:
    ...
    return PACK._canonical_digest(
        PACK.plan_hash_payload(
            ...
            changed_files_sha256=document["changed_files_sha256"],
...
def test_authoritative_consume_pins_the_changed_files_artifact(
    tmp_path: Path,
) -> None:
    plan = _full_plan()
    tested_base = "b" * 40
    ...
    document = plan.to_dict()
    document["changed_from"] = tested_base
    document["changed_files_sha256"] = PACK.changed_files_digest(listed)
    document["changed_files_count"] = len(listed)
    document["plan_sha256"] = _document_plan_sha(document)
    plan_path = tmp_path / "plan.json"
    plan_path.write_text(json.dumps(document), encoding="utf-8")
    changed_files = tmp_path / "changed-files.json"
    changed_files.write_text(
        json.dumps(listed, separators=(",", ":")), encoding="utf-8"
    )
    ...
    assert consumed.changed_paths == tuple(listed)

    changed_files.write_text('["engine/example.py"]', encoding="utf-8")
    with pytest.raises(PACK.ManifestError, match="artifact does not match"):
        PACK.load_authoritative_plan(
            MANIFEST,
            plan_path,
            expected_plan_sha=document["plan_sha256"],
