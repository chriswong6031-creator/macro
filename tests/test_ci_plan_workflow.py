...
def test_ci_plan_job_exists_and_publishes_all_required_bounded_outputs() -> None:
    ...
    three failures can otherwise look green.
    ...
    adding any output that scales with the diff would reopen the hole
    under a new name.
    """
    job = _job("ci-plan")
    assert job["outputs"] == {
...
def test_ci_plan_emits_the_plan_file_and_the_github_outputs() -> None:
    ...
    upload = next(
        ...
        if step.get("name") == "publish authoritative pack plan"
    )
    producer = next(
        step for step in _job("ci-plan")["steps"]
...
def test_structural_preflight_runs_before_expensive_plan_and_uses_exact_paths() -> None:
    ...
    assert "preflight_artifact.write_text(preflight_payload" in body
    assert "artifact.write_text(payload" in body
    assert '--changed-files-file "$RUNNER_TEMP/ci-changed-files/changed-files.json"' in (
        _plan_step()["run"]
    )
...
def test_ci_pack_downloads_and_consumes_the_exact_planner_artifact() -> None:
    ...
    download = next(
        ...
        if step.get("name") == "download authoritative pack plan"
    )
...
def test_ci_pack_pins_plan_sha_head_and_event_base_and_never_runs_unpinned() -> None:
    ...
    assert "CI_CHANGED_FILES_FILE" not in env
...
def test_ci_pack_falls_back_explicitly_to_full_suite_when_planning_cannot_publish() -> None:
    ...
    assert "CI_CHANGED_FILES_FILE" not in fallback["env"]
