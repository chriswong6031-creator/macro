...
class TestPressLane:
    ...
    def test_emission_idempotency_via_seen(self, tmp_path):
        # Run once (live write), collect emitted ids, re-run with them in seen -> 0.
        state1: dict = {}
        ...
        res2 = run_press_tick(_fixture_items(), root=tmp_path, now=_MARKET_HOURS,
                              ...
                              seen_ids=emitted_ids, dry_run=True)
        assert all(e["id"] not in emitted_ids for e in res2["emitted"])
        # the previously-emitted ids come back as dedupe skips
        skip_ids = {s["id"] for s in res2["skipped"] if s["reason"] == "dedupe"}
        assert emitted_ids <= skip_ids
...
class TestDaemon:
    ...
    def test_earnings_lane_default_unchanged(self, monkeypatch):
        ...
        monkeypatch.setattr(d, "_run_one_tick",
                            lambda *, dry_run: earn_calls.__setitem__("n", earn_calls["n"] + 1) or
