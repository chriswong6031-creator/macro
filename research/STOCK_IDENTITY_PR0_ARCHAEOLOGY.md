...
- **Class altitude**: vol×trend terciles and the fundamentals archetype both **failed to separate** from global/random (`:214-218`, `:332-335`) — W4 (per-class gate profiles) is *"UNLICENSED"* pending a grouping variable that actually separates (`:218`). No formal kill row — the door is open to a **different** (e.g. discovered) grouping variable.
...
- Program-registry seam: `config/mastermind_programs.yml` already licenses **`market-timing-intelligence → feeds_context_to → [prophet-us, neural-web]`** while denying it Prophet decision authority (`:938-939`, `:1986-1993`) — the pre-existing lane this program registers under.
...
|---|---|---|---|---|
| `data/stocks/*.parquet` | 229 curated US names | TR-adjusted (`auto_adjust=True`) | Deep (KO→1962, 16,236 rows; NVDA→1999) — close/high/low/volume, **no open** | **Yes — primary** |
