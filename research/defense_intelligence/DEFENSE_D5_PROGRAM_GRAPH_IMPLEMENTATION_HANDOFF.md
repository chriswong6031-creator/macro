...
**Architecture authority:** `DEFENSE_D5_PROGRAM_GRAPH_ARCHITECTURE_FREEZE.md` (same directory) — that document wins on any conflict with this one.
...
2. **All ten adversarial tests T1–T10** (freeze §8) implemented as `gate: code` tests with committed frozen fixtures — never reading nightly-rewritten `site/`/`data/` artifacts (D4 CI-wiring law). T1 (IRDM program-null) and T2 (rename byte-identity) are the two that most often die in review: implement them first.
3. **Virginia-class pilot admitted end-to-end with receipted evidence**: program + capability + platform (Block VI variant) + three role assertions (GD/EB `prime_contractor`, HII/NNS `teammate_subcontractor`, BWXT `supplier` with `shared_scope: true`) + ≥1 milestone (Block VI award 2026-07-29) — every admission backed by an actually-fetched document with sha256 + URL + retrieved_at receipt and a human-review worksheet. **Search-synthesis evidence from D5R (§3 below) is NOT admissible** — re-fetch each document (entitled browser path for 403-hostile .mil hosts) before review.
4. **IRDM negative control live**: the production IRDM route renders `Program relationship: unresolved / not asserted` with `reason_code no_reviewed_exact_path`; every D1–D4 rail byte-unchanged (T1 as a production check, not only a fixture test).
...
6. **Typed states rendered, never blanks** (freeze §6): budget `projection_missing`, GMI rail `not_asserted`, supplier/capability/program `not_asserted` where absent, `conflicted` on conflict rows, plain-word EN/ZH copy keyed off machine enums in the template.
7. **Visual acceptance vs the frozen composition**: per-width (1440/820/390) crops of the real entitled page posted in the PR body against `evidence/compositions/d5-program-dossier-virginia.html`; first screen answers the six questions; no horizontal overflow; inspector-tier IDs below the fold; EN/ZH parity; no translated `title=`.
...
4. T1–T10 test battery green on fixtures.
...
- Do not add `economic_weight`, exposure shares, or ratios to role assertions.
