...
6. **Typed states rendered, never blanks** (freeze §4/§6): budget `projection_missing`, GMI rail `not_asserted`, review-gated rails split `not_reviewed` vs `reviewed_none` (never coerced), `conflicted` on conflict rows, the participants-rail limitation sentence verbatim, plain-word EN/ZH copy keyed off machine enums in the template with rail-scoped copy keys (program rail and atlas rail must never share a copy string).
