...
4. LMT: registrant entity + 14 identifiers rendered without flattening; Sikorsky shown as filing-known but recipient-unobserved; Ex.21 omission clause honored (absence ≠ negative proof).
...
- `unresolved_identifiers[]`: observed recipient identifiers with no reviewed path — value, observed_name, state (`mapping_needed`/`evidence_conflict`), reason (plain words), evidence citations where curated (BWXT trio).
