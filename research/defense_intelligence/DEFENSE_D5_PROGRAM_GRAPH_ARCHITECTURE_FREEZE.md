...
```json
{
 ...
 "review_coverage": [
  ...
  {
   "coverage_id": "rev-cov:67a377c7367f",
   "scope": "program_event_link",
   "subject_type": "award_event",
   "subject_id": "govws-0000000000000000example",
   "known_at": "2026-08-22T08:00:00+00:00",
   "worksheet_ref": "research/government_revenue/PROGRAM_ONTOLOGY_REVIEW_2026-08-22-example.json",
   "worksheet_sha256": "102e96ada371e10d8e84feebcc787ed501bad67808c57f2752766f873239c097",
   "admitted_count": 1
  },
  {
...
**Scope → collection map (frozen; `admitted_count` counts rows admitted in the mapped collections for that scope+subject in that pass):** `program_identity` → `programs` + `platforms` (variant identity is program-native, §3.1a); `capability` → `capabilities` + `program_capability_links`; `participants` → `role_assertions`; `milestones` → `milestones`; `program_event_link` → `program_event_links`. No collection is outside the map.
...
- `program_identity`: `{state, program_id, program_revision, name, phase, sponsor_agency, evidence_refs[]}` — name/phase/sponsor copied from the resolved D5 program revision (D5-owned truth; the ids let the UI deep-link).
- `capability`: `{state, capability_id, capability_revision, name, need_statement, program_capability_link_id, evidence_refs[]}`.
- `awards`: `{source_state, link_state, program_event_link_ids[], event_ids[]}` — pointers ONLY; zero copied event truth (amounts/dates/agencies stay in the event plane, fetched by the UI from that plane's own artifacts).
...
**Latest-block selection (frozen, resolves §9's "latest official program state"):** among the subject program's platform records visible at `as_of` whose validity covers `as_of`, pick the maximum `valid_from`; ties break on the lexicographically greatest `id`.
