...
| `role_assertion` | `prog-role:<sha12>` (content-derived from the normalized tuple `program_id \| platform_id-or-"-" \| entity_id \| role \| role_scope \| valid_from` — `role_scope` and the platform slot are IN the preimage so two assertions differing only in scope or block can never collide) | `program_id` (or `platform_id`), `entity_id` (a defense21 legal-entity id — never a raw name, never a ticker), `role` (closed: `prime_contractor \| teaming_partner \| subcontractor \| supplier` — teaming co-production is NOT subcontracting; D0R F2 keeps JV/consortium distinct and the enum must not fuse them), `role_scope` (evidence-bound prose quoting the source's own words, e.g. "naval nuclear reactor components"), `shared_scope: bool` (true when the evidence sentence covers multiple programs), `single_document_dual_scope: bool`, `economic_weight` (**required, typed null** — name the null, house idiom; role ≠ exposure share), `evidence_refs[]` (`minItems: 1`; loader-enforced coverage rule: the union of the refs' `claim_scopes` must cover BOTH `program_identity` AND `role`; when ONE document covers both, `single_document_dual_scope: true` is required and the review worksheet must quote the exact source-native sentence) | This is the supplier law's carrier, mirroring `government_budget_edge.v1 reviewed_documentary` discipline with the coverage rule made loader-checkable. **Entity attachment rule:** the role attaches to the legal entity the evidence document names as performing the work; a parent-issued release naming a subsidiary's work attaches to the subsidiary. |
...
- **`claim_scopes` enum** on every evidence row: `program_identity | capability_need | role | milestone | ownership_context`. A record's evidence must cover the scopes its kind requires (loader-enforced).
...
| `capability` | D5 ontology | `reviewed` / `not_reviewed` / `reviewed_none` |
...
| `participants` | D5 role assertions × defense21 × atlas | per-row `reviewed` + issuer state (`verified_live` / `listing_terminated` / `not_in_si_universe`); rail-level `not_reviewed` / `reviewed_none`. **Required rendered limitation (verbatim):** "Companies on this rail participate in the same program; no commercial relationship between them is asserted." |
...
| `milestones` | D5 ontology | `reviewed` / `not_reviewed` / `reviewed_none` |
...
New mints (only these, each covering a gap the estate census confirmed has no equivalent): the `role` enum (`prime_contractor \| teaming_partner \| subcontractor \| supplier`), the `succession_reason` enum + `predecessor_id` field, the program-rail `no_reviewed_program_link` reason code, the `not_reviewed` / `reviewed_none` empty-rail split, one `unverified_supplier_language` text-annotation code (extends the closed `_action_text_annotations` family for prose supplier mentions), and a typed `OntologyInputError` for the producer.
...
1. **T1 — IRDM stays program-null.** Given the frozen P00032 EV and zero RAs citing that award: DRM emits `program_link {state: not_reviewed, reason_code: no_reviewed_program_link}` (and, with a pilot worksheet row that covered P00032 and admitted nothing, `state: reviewed_none` — both render the "unresolved / not asserted" plain copy); no program name token renders; government fact bytes unchanged; no `source_published_at` key anywhere; the program rail's rendered copy string is asserted UNEQUAL to the atlas's `no_reviewed_exact_path` copy.
...
3. **T3 — prime role does not smear to siblings.** Given RA(prime) on `legal:X:parent` and two RG subsidiaries with no RA: exposed set = parent only; `economic_weight` is present and typed null on every RA (named null, never a number); the "does not allocate … to an issuer" limitation renders.
...
12. **T12 — co-participation is not a counterparty edge.** Given the full pilot dossier (prime + teaming partner + supplier on one program): the emitted payload contains zero firm→firm edges or adjacency structures of any kind, and the §4 participants-rail limitation sentence renders verbatim (EN and ZH).
13. **T13 — evidence publisher/host refusal.** An evidence row whose `source_url` host is off the §3.1a allowlist (e.g. a press aggregator), or an `issuer_disclosure` row whose host is not the asserting issuer's own pinned IR host, is refused at load with the host named; a mirror-fetched row missing its host-of-record `source_url` is refused.
14. **T14 — role-id collision resistance.** Two admissible RAs identical except `role_scope` (or except `platform_id` Block V vs Block VI) mint DISTINCT `prog-role:` ids; a rebuild with both present leaves each byte-identical.

---

## 9. Experience architecture (Program / Platform Dossier)

Reference composition: `research/defense_intelligence/evidence/compositions/d5-program-dossier-virginia.html` (real pilot data, 1440/820/390 via CSS, shared `d0r-target.css`, sibling of the frozen D1/D2 targets). D0R H composition #5 law applies with two **recorded deviations** (deliberate, D6-bounded — not silent substitutions): (1) D0R's glance element "last GAO/DOT&E" requires GAO/DOT&E sources that are D6 scope; until then the glance carries phase + contract type + latest official program state from admitted milestones, and the GAO/DOT&E element renders as a named gap ("No GAO/DOT&E assessment on file — source not yet collected", `source_unavailable`). (2) D0R's why-rail "EAC / quantity": quantity IS available from admitted milestone evidence (e.g. Block VI = nine boats, official announcement); EAC and all cost figures are D6 (SAR/budget) — the why rail renders quantity + the capability need statement and names the economics gap. Changed = award/milestone (budget events once the budget plane lives); evidence = exact official sources; next = next official milestone.
...
| identity unresolved | participant row whose entity resolves `unresolved`/`candidate_review` via atlas — row renders with identity state chip, no issuer link |
