...
- **Decision taken:** (a) **No intraday act-tier leg wired** — nothing clears the gate. (b) The optional display-only "hidden distribution" badge is **NOT shipped** — at 71% false-alarm with no slow-flush edge it would be UI noise and risk false confidence; the project's honesty discipline ("never let it feed a score") argues for omission, not a caveated badge. The negative result is recorded here instead. (c) **The real unlock is a true sub-daily aggressor-CVD + options-microstructure collector** (taker buy/sell volume from exchange *trade prints*, not candle sign) — scoped as FUTURE work below, not built now: it needs a live exchange feed, accumulates forward (no history for months), and can't be validated until then, so building it speculatively is premature. The candle-proxy screen is the negative-result justification for that future investment.
