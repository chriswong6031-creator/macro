...
(b) **Genesis rate limit**: at most `max_genesis_per_cycle: 1` charter authorized per cycle (new IMMUTABLE budget key), and no new genesis while a prior genesis lobe is in `probation` with zero matured contracts (`genesis_cooldown` — one unproven newborn at a time).
(c) **CHF-family deferral**: until 2026-10-15 (DO_NOT_REBUILD §4 clock), charter proposals token-matching the causal/CHF machine-registration family are denied at the screen.
...
**W2 — Genesis gauntlet hardening.** `kind: "charter"` taxonomy (applier stamp + propose vocabulary + `_item_to_proposal` validation); the R-V6-3 deterministic screen in adjudicate; `max_genesis_per_cycle` + `genesis_accountability_days` budget keys (IMMUTABLE file — human-lane PR); adversary prompt case-law block for charters; tests incl. cap-deny, CHF-deferral-deny, swap-allow, rate-limit.
