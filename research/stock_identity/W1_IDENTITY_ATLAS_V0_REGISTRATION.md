...
- **Dead names (≥5, §13) — logged substitution:** the masterplan draws these from `config/delisted_symbols.yml`, but that ledger holds exactly **2 rows** (CTRA, TPH — both acquired, Form 25-NSE; a W1 discovery, the ledger is a young disclosure instrument). Per §13's own substitution clause ("any missing name is replaced … with the substitution logged"), the W1 dead-name pool = both ledger rows (where present on an allowed plane) ∪ baskets-plane names whose tape ended ≥126 trading days before asof with ≥756 sessions of history (tape-end = ceased-trading proxy; index-exit ≠ death is respected because exit-but-still-trading names keep a live tape). Selection: ledger rows first, then seeded-RNG (seed_dead) draws from the pool until ≥5. `terminated_reason` = ledger reason where available, else `tape_ended (cause unverified)` — honest, disclosed. The production ledger is NOT edited by this program. → `TODO-BUILDER: names + reasons`.
...
- **FIT/TEST boundary declaration: 2020-01-01** (declared here; PTT's handoff doc records no explicit reusable boundary date, checked at W1). Rationale: FIT-side history retains the 2011 / 2015-16 / 2018Q4 cluster mass; the 2020-03 and 2022 clusters stay on the TEST side for later confirmatory reads. Era-law stratum (2010 break, `DNR:LAW-ERA-SPLIT`) remains an independent analysis stratum.
...
- **Sector stratum:** GICS sector from `data/breadth/constituents.parquet` (symbol → name/sector, 503 S&P 500 names); names outside it land in the `UNKNOWN` sector stratum — never dropped.
