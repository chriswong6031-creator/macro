...
- Stressors: MSFT (steady-trender control), META (known epoch-changer), plus
  ...
  - **Secular decliner beyond YELP** (chosen at PR-1 by rule): among `config/delisted_symbols.yml` names present on an allowed plane with ≥1,000 sessions, the one with the deepest close-to-close max drawdown over its final 1,260 sessions (damaged-cohort rule; membership choice is design-touched and never evidence). → `TODO-BUILDER: name + receipt`.
- **Dead names (≥5, §13):** seeded-RNG draw (seed_dead below) among `config/delisted_symbols.yml` names present on an allowed plane with ≥756 sessions of history; `terminated_reason` recorded per name from the ledger. → `TODO-BUILDER: names + reasons`.
...
- **FIT/TEST boundary declaration:** → `TODO-BUILDER-PATCH: adopt the PTT precedent boundary if one exists (scout §12), else declare 2020-01-01`; rationale: FIT-side history retains the 2011 / 2015-16 / 2018Q4 cluster mass; 2020-03 and 2022 clusters stay on the TEST side for later confirmatory reads.
...
→ `TODO-BUILDER: fill per-name table` — for each pilot name: primary plane, first/last date, n_rows, `open` availability, hygiene flags + resolutions, first-print sanity result. Explicit subsections:
- **BABA / AEM / PAAS / WPM / AG deep TR-adjusted presence** (archaeology §6.2 left unverified): verdict per name; if absent from both planes → collection per the CN/HK deep-OHLC template into the program-owned store (§11 below) with plane law note.
- **KRUS depth check:** verdict.
...
→ `TODO-BUILDER-PATCH: exact artifact paths per scout` — sector/industry/cap read from the stock library's published output artifacts (read-only; `scripts/build_stock_library.py` itself is G-8-protected and is neither modified nor executed by this program), realized-vol/ADV computed on-plane. Missing metadata → `UNKNOWN` stratum, never a drop.
...
- If a permitted collection step runs (§9 gate), collected deep OHLCV lands in `data/stock_identity/ohlcv/<SYM>.parquet` (program-owned store, TR-adjusted, `price_plane_id = stock_identity_ohlcv_v1`) — recorded on every downstream row per §4 law vi; flagged in the PR body for operator review. No writes into `data/stocks/` or `data/baskets/` (those planes belong to their own machinery).
