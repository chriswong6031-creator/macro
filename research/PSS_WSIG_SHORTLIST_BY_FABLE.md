...
- **What is RETAINED (display/measurement tier, per the accrual law — a null standalone is not worthless):** the four per-name measurement axes (down-volume persistence half-life; overnight/intraday split-ratio; residual-return autocorrelation; **semivariance-asymmetry — the most alive, the only family to clear its mirror placebo**) stay as codex descriptors and candidate confluence inputs. None was promoted to authority.
- **W-CONF status:** the multi-family confluence wave is MOOT for a 2-of-N among the families (zero standalone survivors). The one non-moot thread is a NARROW **incumbent × semivariance-asymmetry** confluence probe (a validated leg + the one retained input that carries independent information) — available as a future prereg if the operator wants it, not run here.
