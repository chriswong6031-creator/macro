...
- **`site/` must be present.** This worktree's sparse cone excluded it, which silently broke render guards and also produced a false failure in `tests/test_ship_loop_guard.py::test_the_pair_list_is_the_ci_gate_s_own_enumeration`. Adding `site` to the cone (752 MB) turned that test green and is a precondition for any future local run of this lane.
