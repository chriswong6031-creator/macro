...
### 1e. Live production reproduction

<!-- PENDING: live-repro agent (production section order, 55/100-name run, defect confirmation) -->
...
6. **Large-list fragility is real, load-dependent, and silent** (why 55 names "cuts off" while 10 look fine):
   - `watchlist_risk.js:1353-1364` `decorateCards()` fires `SD.loadTicker(t)` for **every** card simultaneously — no batching, no concurrency cap; `stockdata.js:132-140` caches any failure as `null` **permanently** (no retry, no TTL, no error surface) → dropped fetches at 55–100 names lose lanes/badges for the whole session, silently.
   - `market_books.js:195` active-book filter persists in localStorage; a partial filter (e.g. pinned `us`, later HK/CN adds) prints **no disclosure** — the list is just shorter (`watchlist.js:190,202`).
   - `viewItems()` sort is O(n²) (`watchlist.js:214`, `indexOf` in comparator) — jank at 100 names on every render; order-missing tickers silently sort first.
   - `watchstore.js:146-149` selects `watchlist_symbols` with no `.range()` → PostgREST row-cap truncates silently at scale and corrupts the `maxPos` watermark (`:156`).
   - localStorage quota failure (`watchlist.js:116-122`) keeps the in-memory blob → list "works" then vanishes on reload.
   - (No deliberate cap exists: `viewItems()` maps the full array; the only `.slice(0,12)` is the search dropdown.)
7. **Deploy split-brain trap for every JS wave.** The six page JS files are plain-copy template/site pairs (live in ~3 min via VPS pull, CI-guarded) but their `?v=N` stamps are **hand-written** in `watchlist.html.j2:867-878` and only reach production via a render — so a JS body can go live minutes-to-hours before its stamp, and `site/watchlist.html` itself can lag the `.j2`.
8. **Governance drift froze the wrong identity.** Three charters pinned watchlist-page primacy (UWP-R1 "evolution of watchlist.html", PSI §13.1 "stays watchlist.html — NO new page", Books §4 `#wl_auth` "(unchanged)") while `MASTER_PRODUCT_INFORMATION_ARCHITECTURE_V1.md` §10.4 held the page's identity OPEN for Sol/Chairman. Nobody owned the product story; every program added a correct layer to an incoherent whole — precisely the failure mode the CEO handoff names.
