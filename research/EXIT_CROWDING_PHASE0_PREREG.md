# Exit / Crowding Overlay — PHASE-0 PRE-REGISTRATION — **DRAFT (UNRATIFIED)**

> ## ⚠️ STATUS: DRAFT FOR FABLE RATIFICATION — NOT YET REGISTERED ⚠️
> This document is an **Opus draft** authored under roadmap item **P1.3** (research/LIVE_FLOW_PRODUCTION_ROADMAP_BY_FABLE.md, ruling **R3**). Pre-registration is **NOT complete**: no timestamp confers frozen-prereg status until **Fable ratifies** and the ratified version is committed. Do NOT run any join, harness, or gate against this document while it carries the DRAFT banner. It is written to be *ready* to freeze, not to *claim* it is frozen. Every threshold, era split, kill criterion, and FDR slot below is a **proposal** subject to adjudication (mirroring O_OPT_PHASE0_PREREG §0.1's R1–R3 pattern). The word "validated" is used nowhere as a claim; it is CI-guarded and reserved for gated outcomes that do not yet exist.
...
**Author:** Opus research agent, off the frozen SELL base-rate machinery + the live US board ledger schema. **Adjudicator:** Fable (pending). **Runner:** roadmap **P4.4** (RUN happens *after* ratification AND after `data/thetadata_eod/_manifest.json` marks the universe pass complete, per R8).
...
- **L1 — Short-dated OTM call-share spike** (`crowd_call_share_z`).
  Definition: within a held group, the **share of daily options volume in short-dated (≤ 21 DTE) OTM calls** (strike > underlying_price × 1.02), member-mean, standardized to a 252-session rolling z. **Requires the EOD-volume ⋈ greeks join for `underlying_price`** (moneyness needs spot at the greeks timestamp). **Leg window: 2018→** (after the greeks-bearing warm-up: greeks coverage begins 2017 per roadmap F-D licensing note; a full 252-session z baseline needs ≥1 year → first usable z ≈ 2018). Fires when `crowd_call_share_z ≥ +1.5` with 2-session persistence.
