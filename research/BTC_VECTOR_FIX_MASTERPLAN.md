...
Deferred owner decision: **D5 — Class-2 accumulation-sleeve cap** (deep-value evidence softening the gate). Asked only after W1 delivers the breakeven evidence; single pre-committed cap, no ranges.
...
### N5 — Staged auto re-entry (owner-approved D4) — SHIP (decomposed)
- **Calendar opens eligibility:** window start = the thesis monitor's projected window (2026-10-01), replacing `buy_lead_days=0` (which re-enters AFTER the projected bottom).
- **Evidence pulls the trigger (validated signals only):** tranche triggers restricted to (a) MVRV-Z<0 print, (b) weekly close reclaiming the 20w-MA (the one cross-cycle OOS-validated structural trigger from the btc_mtf work), (c) `bottom_pressure` ONLY IF the W1 fire-conditional backtest clears it. 3 tranches (e.g. 40/30/30), auto-fill, owner halt switch.
...
- Class-1 auto-release (N2) and calendar re-entry compose: earliest wins.
...
Calendar-opens (thesis window) + validated triggers (per N5; bottom_pressure only if W1 cleared it) + 3 auto tranches + halt switch + DAT advisory chip + dat_holdings staleness flag. Composes with Class-1.
