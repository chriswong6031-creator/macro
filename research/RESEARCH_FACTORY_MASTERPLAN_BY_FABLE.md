...
The transition helper (`engine/research_factory/state.py`) enforces: allowed-pair matrix, actor-class allowlist, mandatory-field presence, append-only log, and monotonic `as_of`. Anything else raises `IllegalTransition`.
