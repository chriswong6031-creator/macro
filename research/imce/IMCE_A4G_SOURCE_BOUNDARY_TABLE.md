...
- **Interim primary rate leg:** Treasury constant-maturity yields (#13) — `pit_pure`, public domain, no known rights ambiguity, **`V`-grade receipt now on record [AP8, M6]**: the commissioning session obtained it via direct browser access 2026-08-21 (§7 full detail) after two build-worker sessions' `WebFetch` attempts (6 total) timed out against `home.treasury.gov`. PMMS here remains unresolved on RIGHTS (not availability); Treasury CMT is now resolved on availability/PIT-verification but **still unresolved on its specific redistribution/storage terms** — a future A4 registration or ingestion-design session must settle that before relying on it in a registered, PERSISTED cell (no persistent ingestion occurs in this or any prior IMCE wave).
...
| Rate | U.S. Treasury CMT (#13) | `pit_pure`, `V`-grade receipt on record (§2/§7, AP8/M6) — storage/reuse basis still unsettled |
...
**Storage/reuse basis — STILL explicitly NOT settled, unchanged by this receipt:** the receipt above resolves the *availability and PIT-vintage* question (CSV/XML export exists, archived files exist, a dated construction break is now on record) — it does **not** resolve Treasury's specific redistribution/persistence terms, which remain unchecked against Treasury's own terms-of-use page (unlike NAR's and Freddie Mac's terms, independently checked at §4–§5). **No persistent ingestion of Treasury CMT data occurs in this wave or any prior IMCE wave.** A future A4 registration or ingestion-design session must determine and record Treasury's specific reuse terms before any `C_t`/`M_t` field is built and STORED on this series.
