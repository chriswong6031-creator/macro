...
|---|---|---|
| **W1** | Persona spec loader + validation (`engine/marketing/personas.py`), 6 existing desks migrated additively (zero-diff gate), §3.3 codexes (main-loop/Fable authored), admin Persona Roster panel (incl. isolation-checklist + health columns) | Opus `builder`; codexes NOT delegated; panel markup pinned or `designer` |
