...
- **XG-W4 (reply desk phase 1):** not done unless target discovery runs inside the existing twitterapi.io spend cap with since-id cursors, every draft passes the independent critic pass (near-dup vs parent, dignity/screenshot rubric, satire/sensitivity blocklist), the reply queue enforces expiry (stale drafts auto-killed), mode dial ships at M0/M1 only with M2/M3 config-gated OFF, and the M1 desktop runbook + handoff contract are committed.
...
**Adopted wholesale:** Laws 1–10; the four growth loops; the five editorial identities; the six-layer persona model (attention/judgment/taste/position/relationship/language) + three changing states; autobiographical canons and their do-not-invent list; per-persona franchises (§12 concepts become the franchise register in XG-W3); Gift–Grip–Bridge–Proof as the publish gate; informational-surplus, social-object, one-useful-sentence, why-now tests; reply strategy (§9) in full; the anti-sameness discipline; adaptive cadence doctrine (this IS the cadence resolver's requirements spec); negative-feedback budget; relationship memory with its "do not infer sensitive traits" law; measurement families, north-star, parent-adjusted evaluation, bottleneck table; learning-without-collapse (§16); experiments A–H as the standing experiment backlog; the avoid-list; capability requirements 1–15.
...
2. **Persona codexes: pinned v1 signatures are FROZEN; the constitution grafts.** Intelligence Suite masterplan §5 keeps authority over the language layer (register, quirk whitelists, banned lists, emoji signatures, dial examples). The constitution's cognitive layers (perception, judgment, taste, worldview, franchises, canon, restraint clauses) graft on as codex rev-2 in the XG-W1 specs. No contradiction was found per person; if one emerges, §5-pinned wins.
...
| XG-W6 | Telemetry + learning | IS-W5 + parent-adjusted reply labels + blind-identity eval (≥80% target) + persona health monitor + network tripwire (Persona W2b, moved here — measurement infrastructure, not a posting precondition) + last-nine-post diagnostic; ALL feedback-loop charters converge here (D05 §7 loop ≡ IS-W5 ≡ Persona W4 report card ≡ Media W2 scorecards — one loop, four consumers) | IS-W5 + Persona W2b/W4 + Media W2 measurement slice | Opus `builder` |
...
5. Standing from prior charters: DNS for the press properties; golden-set labeling session; NewsAPI.ai $90/mo call; press-feeds arming flips.
