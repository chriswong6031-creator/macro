...
**Surface:** the existing Milestones tab on the BioCatalyst page graduates into **Catalyst Radar — Trial Milestones** (same page, same entitlement, no new route family). Rows ordered by time-to-event. Each row (glance tier): trial short title · phase chip · issuer chip (ticker when resolved) · event kind (plain words: "Primary completion" / "Study completion") · scheduled date with honest precision AND registry date type (Est./Actual) · days-to-event · a **date-moved chip** when revision history exists ("moved +3 mo · Aug 12") · condition. Hover/expand (tier 2): full revision lineage, known_at stamps, sponsor line, intervention name, link to trial dossier, **evidence drill-down** to the stored source receipt and record-history versions.
