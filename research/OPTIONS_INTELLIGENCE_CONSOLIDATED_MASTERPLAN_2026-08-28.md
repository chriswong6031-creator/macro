...
|---|---|---|---|
| A | **AD-1T2**: natural store-bearing ThetaData → intel brief → existing Options Workspace production acceptance | Two conditions per the owner record (`WS-ADVANCED-DATA-OPTIONS.md` AD-1T2 row): (i) T1 cadence production-proven ✅ (two consecutive natural sessions, receipted 08-25); (ii) **Sol acceptance of AD-1T1 — UNRECEIPTED**, requested as ruling R3 in §15 | **YES once R3 lands** (dispatch C1 with the acceptance in its key) |
...
**Recommendation: ADOPT, under seven conditions A0–A6 — without rewriting its out-of-order history.**
...
- **A0.** The Chairman written-spec gate is discharged with a receipt. `DEC:OPTIONS-ALPHA-CAMPAIGN-CALIBRATION-ARCHITECTURE` (merged in #6573) states: "No implementation wave begins until the Chairman separately approves the written spec." Under this program's own precedence (§1-3) that merged source decision outranks dispatch prose, and under the authority ladder (§6) a Sol ruling cannot by itself discharge a Chairman condition. Required: a receipted Chairman approval of `docs/superpowers/specs/2026-08-27-options-alpha-intelligence-recovery-design.md` (it may already exist outside the repo — if so, cite it into the record), or an explicit receipted Chairman waiver. Until then the gate is recorded **UNSATISFIED** (§14-b).
...
- **Dispatch gate:** ruling R3 (Sol acceptance of AD-1T1) must be receipted in the dispatch; the owner record requires it before AD-1T2 opens.
...
- **Sequencing:** A0 receipt (Chairman approval/waiver) cited into the record → C0 merges first (§8-2) → #6576 rebase+merge (A1) → A6 line-level diff review by an independent Opus `reviewer`, findings posted on #6585 (a FAIL stops C3 and returns to Sol) → §11 docket receipts on #6585 (A2) → hold release per Sol → #6585 merge (A3/A4) → records closeout recording BUILT_NOT_PROVEN + proof-owed (A5).
...
**Rulings this carrier explicitly requests from Sol (beyond CONTINUE/REQUEST_REPAIR/STOP):**
- **R1.** The §10 adjudication of #6585 (ADOPT under A0–A6, or REJECT with its stated cost), including the A0 disposition: cite a Chairman written-spec approval receipt, obtain a Chairman waiver, or record an explicit CEO override of the Agent OS record.
- **R2.** Ratification (or correction) of the §5 "Terminal producer authority" reading (§14-d) and the §7 registry recommendation (fifth-owner pointer update, executed by a later records child).
- **R3.** Explicit Sol acceptance of AD-1T1 (the owner record requires it before AD-1T2 opens; the production packet is PR #6267 issuecomment-5419508761). Lane A / child C1 dispatch waits on this.
- **R4.** Disposition of the pre-existing validate red (§8-7): authorize its owning workstream (or a named child) to repair `agentos/handoffs/BREATHING-PLATFORM-2026-08-28-completion-commission.md`.
