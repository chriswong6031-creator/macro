...
## LOOP DISCIPLINE
- Each round: pick 2-3 signal FAMILIES (rotate so the union diversifies — don't re-mill washout+credit every round), have a Sonnet subagent GENERATE ~30-50 grammatically-valid first-principles specs in those families + ingest + reversion-screen + gauntlet the passers, and REPORT the full-pass validated ones. You adjudicate, append passes to the validated base, choose next round's families.
- The ingest hard-dedups exact rules, so re-proposals are cheap/safe. Log honestly.
- Pre-registration is sacred: the gate thresholds are FROZEN — do not tune them to pass a candidate.
