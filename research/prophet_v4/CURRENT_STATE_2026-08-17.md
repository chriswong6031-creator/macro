...
- **B-15…B-19 defect labels** (masterplan §8.3) resolve to the independent audit `research/US_PROPHET_COMMERCIAL_LAUNCH_READINESS_2026-08.md` §5.4/§8 against #5370's early-turn union: **B-15** open-3D-bucket repaint at admission (STLD 8 fires→4 survive; `engine/us_early_turn.py:651-653,835,1102-1103`); **B-16** manifest scheduled-red/schema-version consistency; **B-17** shipped deck ≠ measured object (union∩confirmed-lane vs naked-union numbers; `prophet_bridge.py:4018,4318-4319`); **B-18** deck⊇plan invariant inverted (114 STLD sessions `fired=True, deck_admitted=False`, `admission_era: None`); **B-19** chase verdict fires off a dead union fire (`us_early_turn.py:948`). Also: stage literals CONFIRMING/CONFIRMED are declared but unreachable (only EARLY ever assigned, `:886-887,1093`). Current disposition of each is V4-B2's opening task; do not assume the #5370 heal closed them.
...
1. **Engine stage** — `engine/us_board_rank.py:418-432`: `_LIVE_STATUSES = frozenset(("buy_now","partial","buy_soon"))` (`:429`) buckets `buy_soon` rows into `STAGE_LIVE` → heading **"Live now — entry window is open now"** (`:443-448`); drives board headings + `data-stage` filters (`templates/dashboard.html.j2:15968-15981,16207`).
...
4. **Int-valued stage rail** — `dashboard.html.j2:16035` `_STAGE_BY_LANE` duplicate drives the card's 4-stage "Bottoming→Turning→Ready→Trend" tracker (`_prophet_card.html.j2:11`), also lane-derived.
...
**A design-ratified fix already exists, unexecuted:** `research/migration_packets/MP-1-prophet-board.md` (2026-08-13, design-authority-authored per #5504/#5505) prescribes a 7-cell plan-lifecycle ladder (watch/ready/entered/delivering/overtime/invalidated/resolved), orders the RIPENING table chip RETIRED and the `_STAGE_BY_LANE` duplicate REMOVED, names re-sourcing the card population from `_su.buy` to the plan book (`site/prophet/index.json.plans`) as the migration's "structural act", and **all three of its spawn gates are satisfied today** (G-A `lifecycle_counts` emitted by `build_prophet.py:2482-2484`; G-B `.mx-ladder` live in `theme.css:1940`, Sol §J.9; G-C frozen R3/R4 crop set in `mockups/refs/institutionalize/us_stocks/`). Nothing in the template reflects it yet. V4-B3/B5/E2 build against MP-1 rather than re-designing (reconciliation ruling: `ARCHITECTURE_FREEZE.md` §12).

**Auth/delivery gap:** the page is public; tier caps are a DOM-visibility overlay (`templates/tier_preview.js:23-26,198-213` — anon 1 row, free 3, paid unlimited via blur/`aria-hidden`), while the FULL board (cards + table JSON) is unconditionally SSR-baked into page source (`dashboard.html.j2:15796-15825,16001-16002`). Server-side withholding exists as a house pattern (ETF desk `premiumdata/etfs.json`, `scripts/build_site.py:3016,3100,3164`) and MP-1 §7 assumes `premiumdata/us_stocks.json` — which does not exist. Anonymous visitors can read the full board from source today.
...
18. Table view excludes watch/leaders/laggards/ran — All Candidates (B5/E2) replaces it as the complete surface.
