...
- **Owned paths while active (Lane A holds the publication plane alone):** `daily.yml` Prophet-relevant steps, `scripts/prophet_rescue.py`/`check_nightly_liveness.py` only if the root cause lives there, `scripts/build_prophet.py`/checkpoint plumbing as needed. Coordinate = nothing else touches these until you merge.
