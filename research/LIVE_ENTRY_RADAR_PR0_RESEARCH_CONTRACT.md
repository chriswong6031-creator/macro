...
3. **Role of outcomes:** in the kill, outcomes *manufacture the label* (the winner). In the intended construction, outcomes only *test the routing law* — exactly the CEO's stated form, and exactly how W1b was graded.
...
(i) **Name-disjoint law validation:** fit the routing law on a FIT name-set; grade on disjoint names × disjoint eras (DT-R16). The killed construction cannot even run off-name — transport is evidence audition cannot counterfeit. (ii) **Audition negative control:** run W1a-style per-name audition as a mandatory control arm (expected ≈ random per the kill's two-ruler numbers); the structural router must beat both random AND the audition control on the same ruler — a router that tracks the audition arm is a disguised audition. (iii) **Outcome-permutation invariance:** permute per-name outcome histories holding fingerprints fixed → routing assignments must be bit-identical (mechanical test; an audition-in-disguise changes assignments). (iv) **Epoch-shift following:** on identity-epoch breaks the routed expert must follow the *new* fingerprint under the shared law, while an audition key follows the ticker's old winner — grade the two predictions on post-break episodes. (v) **Ruler robustness:** any routing claim states its ruler and replicates ordering under both the forward-window and §7 timing rulers (per the home-rung ruler-dependence finding).
...
3. **Routing/conditioning features must be PIT-measurable structure** (bars, state, episode morphology), enumerated in a frozen feature registry whose hash rides on the qledger claim; any feature that is a function of own-name outcomes is rejected at registration.
4. **The A2.4(iii) permutation-invariance test ships as CI** with any routing implementation — the guarantee is mechanical, not editorial.
5. **Name-disjoint cross-validation for any fitted law; the audition negative control mandatory in every promotion read; TrialLedger look-counting throughout.**
6. **Cohort-granularity floor:** every conditioning cell must contain ≥ a pre-registered minimum of distinct *names* — a "cohort" can never shrink to one ticker through a side door.

**A2.6 — VERDICT: PROCEED, not REJECT.** The Identity → State → Episode → Expert hypothesis is structurally distinct from the killed construction on parameter structure, feature causality, the role of outcomes, and transportability — and is the direct generalization of the carve-out arm the kill's own evidence upgraded. It proceeds under the A2.5 guarantees as binding law, with W1b's unearned-engine-seat status inherited honestly. Two Radar-side applications take effect in this contract: the §9 side-door prohibition (A2.5.1–2), and the **R-W1T-3 copy law inherited into §14**: user-facing Radar copy says *reset confirmed / washout turn*, never *bottom called* — which is also the measured product truth of this tool family (called-low 5–8% vs 8.4% ambient; confirmed-reset 48–66%).
