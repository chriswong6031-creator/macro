...
**No minute/hourly/4H U.S. equity bar store is built in the Macro repo today** (the only hourly equity store is 15-min-delayed, curated-universe — wrong grain, latency, and universe for replay). **The entitlement, however, is confirmed**: Massive flat-file `minute_aggs_v1` from ≥2010-06-15, tick history to 2005, real-time REST/snapshots — with the masterplan's own strategy law: *episode-windowed files + per-name REST, never a bulk crawl* (a blanket multi-year backfill is TB-scale). Therefore:
