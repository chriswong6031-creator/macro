...
Radar adopts the same mechanism for its oscillator-state conditions: StochRSI-K (and K×D relations) as a function of *today's provisional close* is monotone/piecewise-monotone, so **C1's arm condition (K<20) and C2's cross conditions invert nightly into per-name price thresholds** evaluated every 5 minutes against delayed snapshots — no intraday indicator recomputation, no new bar feed, full parity with the confirmed-bar math by construction. Path-dependent turn features that thresholds cannot encode (session low, rebound-in-ATR from it, intraday K path extremes) are derived from the same 5-minute snapshot stream and are recorded as **path observations, not bar math**. This is the PR-4 baseline architecture; anything requiring true intraday bars is deferred to the vendor decision below.

### 7.2 The 4H tier is contingent (Track B §4 blocking finding)

**No minute/hourly/4H U.S. equity bar store exists in the Macro repo today** (only BTC/commodity hourly; the live equity plane is delayed quote snapshots). Therefore: `C3_1D_4H_RECOVERY` and minute-level replay of 1D-LIVE observations are **contingent on the vendor answer** (Track D: Massive entitlement to historical minute aggregates and a sanctioned live path). If the entitlement is real and the data lane demonstrates cadence/constraints/load, C3 and minute-replay proceed per §5; if not, C3 is deferred by amendment and C1/C2 run **live-forward only** for their LIVE-state forms (their confirmed-bar forms replay from daily bars regardless). Any new intraday feed carries its own adjustment-basis reconciliation against the adjusted daily plane (§5 price-basis row).
...
**NC-2 proximity kill-arm (inherited from RUL-28, mandatory):** every primary comparison below is additionally read under a proximity de-confound arm (63-bar close-min proximity band fixed-effects, or the closest feasible equivalent registered at PR-5): an excess that dies when proximity-to-recent-low is controlled is a **proximity shadow** and is reported as such, never as detector edge. This is the exact instrument that killed the 2W washout×turn seed; Radar runs it on itself by construction.
