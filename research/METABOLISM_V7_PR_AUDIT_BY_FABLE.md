...
**R-V7-6 — Refusals.** The auditor authors nothing (read-only review; it never edits the diff — a rejected PR goes back as a fresh proposal next cycle, or to operator tap, never auto-patched by the auditor). No bypass of the audit gate for any tier. No LLM-originated approval outside the deterministic envelope. The audit does not replace VERIFY (fitness grading, post-merge) — it is pre-merge code correctness/containment, a different question.
