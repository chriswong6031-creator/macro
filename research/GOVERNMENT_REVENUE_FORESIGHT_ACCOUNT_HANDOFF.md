...
- The Wave 7 checkpoint passes 338 focused Government Revenue/build/API/UI/Prophet/Neural Web and asset-contract tests plus template/site sync, JavaScript syntax, projection, and raw edge-budget guards. Its three dependency bundles use the shared `data-sync` loader contract so post-render cache stamping cannot defer them beyond the inline page bootstrap; the committed page keeps only stable loader/link changes while governed render lanes rebuild the volatile payload and full body.
