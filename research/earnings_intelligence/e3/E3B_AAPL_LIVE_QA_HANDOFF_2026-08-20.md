...
**Wave:** E3-B · **Date:** 2026-08-20 · **Authority:** `E3_EVENT_INTELLIGENCE_COMPILER_FREEZE_2026-08-20.md`  
**Depends on:** E3-A hard gates green on the frozen AAPL gold.  
**Do not start from E3-0.**

Not done unless production AAPL Intelligence and the public glance consume **accepted** `qa_exchanges[]` for `evt_cik0000320193_2026q3_results` without regressing the E2 event to CI v1.

---

## Mission

Promote validator-accepted `qa_exchange.v1` objects into the existing `event_workspace.v1` `qa_exchanges` list for the live AAPL FY2026 Q3 event. Terminal and the Macro dossier must read those objects. Empty list remains the honest state if validation admits nothing.

## Visible user add

- Terminal Intelligence / Results: structured Q&A (ordered question/answer, questioner/respondent when source-supported, closed topic labels). Stop treating overlay `14` or `facts.questions_count` unstructured absence as the live Q&A once accepted exchanges exist.
...
Same `event_id` + new transcript/release SHA → invalidate affected exchanges → rerun Q&A family only → new `generation_id`. Do not fork a second AAPL event.
...
- `engine/company_intelligence/event_workspace_build.py` (stop hardcoding `[]` once accepted objects exist; keep `[]` when none)
...
- Live AAPL workspace generation advances with accepted `qa_exchanges` **or** stays empty with an explicit validation receipt that nothing replayed.
- Authenticated Terminal AAPL Intelligence at 1440 EN / 820 EN / 390 ZH shows the structured Q&A without overflow or CI console errors.
- Public AAPL dossier does not request `/api/company-intelligence/AAPL`.
- LMND still falls through on canonical 404.
- Beat/miss still absent. Prophet flags still false. `authority=context_only`.
- Native or explicit-unknown clocks on accepted exchanges; no `generated_at` used as `source_available_at`.

## Out of scope

Second issuer, natural-cycle event, slides, consensus, reaction join, deflection scoring, earnings_qual score schema, FIF-2B/7.
