...
next_actions:
  - "RESUME CONDITION (Sol, 2026-08-19 — the seat stops rather than polls): resume when #5933 + #5943 + #5955 are merged to main. On resume: rebase this PR's branch (claude/china-alpha-masterplan-2026-08-19) onto the resulting canonical main; verify exactly ONE canonical US G0 (research/earnings_intelligence/g0/) and ONE CN-G0 (research/alpha_intelligence/censuses/CN-G0/) exist on main; verify WS-ALPHA-INTELLIGENCE-INTEGRATION obtains c0/c0g done FROM MAIN (not from this PR); run python3 scripts/agentos.py validate plus a stale-reference sweep (censuses/G0/, C0G_G0_ADJUDICATION non-SEAT, '#5953 owns US G0'); keep #5953 DISARMED; return the exact rebased head SHA to Sol for final freeze review."
  - "GATE: Sol's final freeze review of the rebased #5953 head — no arm, no merge, no spawn before it clears."
  - "Sol's two precision hardenings are ALREADY APPLIED on this head (PR-0D: USCC/LEI = external resolution keys into canonical Data OS identity, never replacement IDs; P1: failure isolation is loud — typed degraded/failed health, no false measured_no_event) — do not re-apply at rebase."
