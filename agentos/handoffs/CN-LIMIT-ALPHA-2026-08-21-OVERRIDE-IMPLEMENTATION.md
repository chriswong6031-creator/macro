...
unresolved:
  - "DEP-EXACT is now TECHNICAL_CANARY_REQUIRED and its next action is a technical dispatch (mode=plan, then one bounded mode=execute canary window on .github/workflows/tushare-spine-backfill.yml). This PR does NOT run it, does not flip BULK_HISTORICAL_BACKFILL_READY, and takes no position on when the canary should be scheduled."
...
next_actions:
  ...
  - "After merge: dispatch tushare-spine-backfill.yml mode=plan (network-free) to confirm lane, store path, and resume plan; then ONE bounded mode=execute canary window with parity/throughput/error-taxonomy receipts."
  - "Only on those receipts may a separate reviewed change flip BULK_HISTORICAL_BACKFILL_READY; then the range campaign and the sanitized completeness manifest close DEP-EXACT."
do_not_redo:
