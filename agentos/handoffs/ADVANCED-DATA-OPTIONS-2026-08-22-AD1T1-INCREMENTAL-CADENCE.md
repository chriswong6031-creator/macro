...
changed:
  ...
  - {path: "tests/test_topup_thetadata_legacy.py + test_topup_thetadata_daily.py + test_topup_thetadata_scheduler.py", what: "133 tests — characterization-first legacy pin + full §H hostile families + reviewer-round regression pins"}
verified:
...
unresolved:
  - "Sol ruling requested on two ensure-law-adjacent amendments: (1) S-panel healthy split (healthy gates on EOD[S]∧Greeks[S]∧OI[S] >= 0.90; chain_next OI[D] reported separately, never gating); (2) fetch_failed taxonomy superset replacing timeout_or_stream_failure (collector None is cause-blind; >25% triggers a reachable() re-probe / terminal_lost_mid_run abort)."
...
next_actions:
  - "Sol: review PR #6267 (DRAFT, HOLD-FOR-SOL); rule on the two amendments above; accept or amend the W=4 concurrency ruling and the scheduler transition."
