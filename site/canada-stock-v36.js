...
(function () {
  ...
  function leadRow(x, max) {
    /* Breadth truth (Sol R3, unitless gauge R3-02): null/undefined count is
       UNKNOWN membership (the row that displays "—") and gets NO meter at
       all — a floor here would paint an unknown group as if it had
       measured breadth. A true zero count gets an explicit 0 (no visible
       fill, but honestly zero). Anything else renders its real proportion
       with no minimum floor. --breadth is emitted UNITLESS (an integer 0-
       100, not a percentage string) so the bounded-gauge scale constant
       lives in CSS (--mx-breadth-unit), not duplicated here. A row that
       carries a meter also gets the ca-v36-has-breadth marker class so the
       CSS can suppress the gauge TRACK on unknown-membership rows instead
       of rendering an empty track for a group whose breadth was never
       measured. */
