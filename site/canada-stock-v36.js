...
(function () {
    ...
    laneDefs.forEach(function (def) {
      qsa(def[0] + " .anv2-row").forEach(function (node) {
        ...
        out.push({ kind: "sector", rank: out.length + 1, id: id, name: name,
          stance: { en: def[1], zh: def[2] }, tone: def[3], count: members.size,
  ...
  function openModal() {
    ...
    qs("#ca-v36-modal-body", modal).innerHTML = '<div class="ca-v36-modal-grid">' + modalPane(state.themes, bi("Theme Leadership", "主题领先"), bi("Names", "成分")) + modalPane(state.sectors, bi("Sector Leadership", "板块领先"), bi("Board", "榜单")) + '</div>';
