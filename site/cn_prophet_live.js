...
(function () {
  ...
  function paintStrip(d, nowCst) {
    ...
    var have = Number(isClose && isFinite(Number(_cbN)) ? _cbN : cov.observable_n);
    var tot = Number(cov.armed_n);
    if (isFinite(have) && isFinite(tot) && tot > 0) {
      r1 += sep() +
        '<span class="l-en"><span class="cnlv-n">' + esc(have) + "</span> of <span class=\"cnlv-n\">" +
          esc(tot) + "</span> " + (isClose ? "closes in" : "quoted") + "</span>" +
        '<span class="l-zh"><span class="cnlv-n">' + esc(tot) + "</span>只中已到 <span class=\"cnlv-n\">" +
          esc(have) + "</span>只</span>";
    }
