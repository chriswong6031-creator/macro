...
(function () {
  ...
  '.mmacc-alerts::before{content:"";position:absolute;left:0;top:13px;bottom:13px;width:2px;border-radius:var(--r-ctl,8px);background:var(--line);transition:background .22s ease,box-shadow .22s ease}' +
  '.mmacc-alerts[data-on="true"]::before{background:linear-gradient(180deg,var(--ok),color-mix(in srgb,var(--ok) 26%,transparent));box-shadow:0 0 10px -3px color-mix(in srgb,var(--ok) 55%,transparent)}' +
  '.mmacc-alerts[data-state="unknown"]::before{background:color-mix(in srgb,var(--muted) 40%,transparent);box-shadow:none}' +
  '.mmacc-alerts[data-quiet="true"]::after{content:"";position:absolute;left:0;bottom:13px;width:2px;height:34%;border-radius:var(--r-ctl,8px);background:repeating-linear-gradient(180deg,color-mix(in srgb,var(--ok) 70%,transparent) 0 4px,transparent 4px 8px)}' +
  '.mmacc-k-lead{color:var(--text);font-weight:600}' +
  '.mmacc-sublabel{font-size:var(--fs-label,11px);font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:var(--muted);margin:12px 0 6px}' +
  'button.mmacc-switch{width:34px;height:20px;border-radius:var(--r-pill,999px);background:var(--panel2);border:1px solid var(--line);position:relative;flex:none;padding:0;cursor:pointer;opacity:1;font-family:var(--font-ui,inherit);transition:background .18s ease,border-color .18s ease}' +
  'button.mmacc-switch::after{content:"";position:absolute;top:2px;left:2px;width:14px;height:14px;border-radius:var(--r-pill,999px);background:var(--muted);transition:transform .18s cubic-bezier(.2,.9,.3,1.1),background .18s ease}' +
  'button.mmacc-switch[aria-checked="true"]{background:var(--ok);border-color:var(--ok)}' +
  'button.mmacc-switch[aria-checked="true"]::after{transform:translateX(14px);background:var(--panel)}' +
  'button.mmacc-switch:focus-visible{outline:none;box-shadow:0 0 0 3px color-mix(in srgb,var(--link) 22%,transparent)}' +
  '.mmacc-switch-sm{width:30px;height:18px}' +
  '.mmacc-switch-sm::after{width:12px;height:12px}' +
  '.mmacc-switch-sm[aria-checked="true"]::after{transform:translateX(12px)}' +
  '.mmacc-alert-detail{max-height:0}' +
  '.mmacc-alert-detail.open{max-height:420px}' +
  '.mmacc-select{appearance:none;background-image:none;cursor:pointer}' +
  '.mmacc-timepair{display:flex;flex-wrap:wrap;align-items:center;gap:8px}' +
  '.mmacc-time{width:auto;min-width:0;flex:1 1 108px;margin-bottom:0}' +
  '.mmacc-timesep{color:var(--muted);font-size:var(--fs-sm,12.5px);flex:none}' +
  '.mmacc-hint-off,.mmacc-hint-unknown,.mmacc-hint-none{margin-top:7px}' +
  '.mmacc-hint-unknown{color:var(--ink-warn,var(--warn))}' +
  ':root[data-theme="light"] .mmacc-alerts{background:var(--panel);border:1px solid var(--line);border-radius:var(--r-card,12px);padding:11px 12px 11px 14px;box-shadow:var(--card-shadow)}' +
  ':root[data-theme="light"] .mmacc-alerts[data-on="true"]::before{background:var(--ok);box-shadow:none}' +
  ':root[data-theme="light"] button.mmacc-switch[aria-checked="true"]::after{background:var(--panel);border:1px solid color-mix(in srgb,var(--text) 12%,transparent)}' +
  ':root[data-theme="light"] .mmacc-time,:root[data-theme="light"] .mmacc-select{background:var(--panel)}' +
  '@media (max-width:560px){.mmacc-timepair{gap:6px}.mmacc-time{flex:1 1 100%}.mmacc-timesep{display:none}}' +
  '@media (prefers-reduced-motion:reduce){.mmacc-alerts::before,button.mmacc-switch,button.mmacc-switch::after{transition:none}}';
