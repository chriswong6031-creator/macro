...
# Pre-existing HARD gaps, pinned so the guard is green on the tree it landed
# against while the set stays CLOSED — anything new fails immediately. Each
# needs its own builder-side fix; the sweep that added this file was scoped to
# reference-path integrity. Shrink this list; never extend it without the
# matching emitter fix.
_KNOWN_GAPS: frozenset[str] = frozenset((
    # Retired pages still linked from the HK dashboard.
    "heatmap.html",
    "sectors.html",
    # A basket id no longer in data/baskets/membership.json, so no page is
    # built for it — but baskets.html + us_stocks.html still link one.
    "basket/ai_capex.html",
    # Research adjudications linked as /research/<name>.md from signal_lab.html.
    # The files exist in the repo's research/ tree but are never copied into
    # site/research/, so the links 404 in production.
    "research/SIGNAL_LAB_FRONTIER_WAVE2_FABLE_ADJUDICATION_2026-07-06.md",
    "research/SIGNAL_LAB_FRONTIER_WAVE3_FABLE_ADJUDICATION_2026-07-06.md",
    "research/SIGNAL_LAB_FRONTIER_WAVE4_FABLE_ADJUDICATION_2026-07-06.md",
))
...
def _selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        (site / "stocks" / "OK.html").write_text(
        ...
        (site / "stocks" / "BAD.html").write_text(
