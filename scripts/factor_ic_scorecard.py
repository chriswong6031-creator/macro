...
FACTOR_COLS = list(FACTOR_LABELS) + ["composite"]
...
def main() -> int:
    ...
    if last_table is not None:
        fcols = [c for c in FACTOR_COLS if c in last_table.columns and c != "composite"]
        X = last_table[fcols].apply(pd.to_numeric, errors="coerce")
        coll = {"vif": vif(X), "top_pairs": top_correlated_pairs(X, k=6, thresh=0.5)}
...
def _write_md(report: dict) -> None:
    ...
    if report.get("collinearity", {}).get("top_pairs"):
        L.append("## Collinearity (latest cross-section)\n")
        L.append("VIF>5 = redundant. Correlated factor pairs:")
        for p in report["collinearity"]["top_pairs"]:
            L.append(f"- {p['a']} ↔ {p['b']}: |corr| {p['corr']}")
        hi = {k: v for k, v in report["collinearity"].get("vif", {}).items() if v and v > 5}
        if hi:
            L.append(f"\nHigh VIF: {hi}")
    L.append(f"\n> {report['note']}")
