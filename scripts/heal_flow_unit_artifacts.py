"""Heal flow-column unit artifacts in the committed EDGAR fundamentals panel.
...
Columns repaired: ni, cfo, gross_profit, revenue, op_income, dividends, repurchases.
See collectors/edgar_flow_quality.py for the full detection/repair design.

Idempotent; run from any checkout that has the panel:

    python -m scripts.heal_flow_unit_artifacts [--dry-run]

Writes the healed parquet in place, a full audit to
data/edgar/_flow_quality_audit.json, and a flow_quality block into
...
_STMT_COLS = ["ticker", "fy", "ni", "revenue", "cfo", "op_income", "gross_profit"]
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        meta = {}
    meta["flow_quality"] = {k: audit[k] for k in
