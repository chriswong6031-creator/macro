...
def _wait_review_date(value: Any) -> _dt.date | None:
    """The intentional-wait review clock is date-only, never a timestamp or timer."""
    if isinstance(value, _dt.datetime):
        return None
    if isinstance(value, _dt.date):
        return value
...
def _check_wait(value: Any, *, where: str, path: Path, out: list[Problem]) -> None:
    """Validate optional intentional inactivity without granting it execution authority."""
    if value is None:
        return
    ...
    if unknown:
        out.append(
            Problem(
                path,
                "bad-wait",
                f"{where} wait has unknown field(s): {', '.join(unknown)}",
                hard=True,
            )
        )
    kind = value.get("kind")
    if kind not in WAIT_KINDS:
        out.append(
            Problem(
                path,
                "bad-wait",
                f"{where} wait kind {kind!r} is not registered; allowed: "
                f"{', '.join(sorted(WAIT_KINDS))}",
                hard=True,
            )
        )
    if _wait_review_date(value.get("review_after")) is None:
        out.append(
            Problem(
                path,
                "bad-wait",
                f"{where} wait review_after must be YYYY-MM-DD",
                hard=True,
            )
        )
    ...
    if not isinstance(condition, str) or not condition.strip():
        out.append(
            Problem(
                path,
                "bad-wait",
                f"{where} wait condition must be a non-empty string",
                hard=True,
            )
        )
...
def _wait_json(value: Any) -> dict[str, str] | None:
    """Normalize a valid authored wait for JSON views; never infer or interpret it."""
    if not isinstance(value, dict) or set(value) != _WAIT_KEYS:
        return None
    kind = value.get("kind")
    review = _wait_review_date(value.get("review_after"))
    condition = value.get("condition")
    if kind not in WAIT_KINDS or review is None:
        return None
    if not isinstance(condition, str) or not condition.strip():
        return None
    return {
...
def check_workstream(rec: dict[str, Any], path: Path, programs: set[str] | None) -> list[Problem]:
    ...
    if isinstance(waves, list):
        ...
        for index, wave in enumerate(waves):
            ...
            _check_wait(wave.get("wait"), where=f"{where} ({wid})", path=path, out=out)
...
def scan_worktrees(degraded: Degraded, *, deep: bool = False) -> dict[str, Any]:
    ...
    return {"count": len(branches), "branches": sorted(branches), "uncommitted": uncommitted}
...
def _pr_index(builds: dict[str, Any] | None) -> dict[int, dict[str, Any]]:
    ...
    for pr in builds.get("open_prs") or []:
        ...
        if isinstance(number, int):
            index[number] = {"number": number, "state": "open", "title": pr.get("title", ""),
                             "merged_at": None, "is_draft": bool(pr.get("is_draft")),
                             "merge_state": pr.get("merge_state", "")}
    for pr in builds.get("recent_merged") or []:
        number = pr.get("number")
        if isinstance(number, int):
            index[number] = {"number": number, "state": "merged", "title": pr.get("title", ""),
                             "merged_at": pr.get("merged_at"), "is_draft": False,
                             "merge_state": "MERGED"}
    return index
...
def build_records(
    store: Store, *, now: _dt.datetime, builds: dict[str, Any] | None,
    p0_status: dict[str, str] | None, worktrees: dict[str, Any],
    ...
    for key in sorted(ws):
        ...
        for wave in waves:
            ...
            for number in wave_prs:
                ...
                if state == "merged" and merged_at:
                    done_at = merged_at
                pr_rows.append({"number": number, "state": state, "wave": wave.get("id"),
                                "merged_at": merged_at})
            ...
            wave_detail.append({
                "id": wave.get("id"), "title": wave.get("title"), "status": wstatus,
                "depends_on": deps, "deps_satisfied": all(d in done_ids for d in deps),
                "next_action": wave.get("next_action"), "wait": _wait_json(wave.get("wait")),
                "prs": wave_prs, "done_at": done_at,
            })
        ...
        if isinstance(needs, dict):
            blocked_waves = sum(1 for w in waves if w.get("status") in {"todo", "in_progress"})
            needs_row = {
        ...
        out.append({
            "key": key, "title": str(rec.get("title") or "").strip(), "status": status,
            "program": rec.get("program"), "p0": p0 if isinstance(p0, str) else None,
            "p0_active": p0_active, "owner": rec.get("owner"), "class": rec.get("class"),
            ...
            "created": created, "updated": updated, "stale_days": stale_days,
            "depends_on": _refs(rec.get("depends_on"), "WS"), "blocked_by": blocked_by,
            "waves": rollup, "wave_detail": wave_detail, "prs": pr_rows, "claim": claim_row,
            "needs_ceo": needs_row, "collisions": [], "warnings": record_warnings,
            "source": _rel(path),
        })
    ...
    for row in out:
        ...
        for other_key in sorted(owns):
            if other_key == row["key"]:
                continue
            shared = sorted({a for a in mine for b in owns[other_key] if _owns_overlap(a, b)})
            if shared:
...
def compute_readiness(
    ...
    done_workstreams = {key for key, row in by_key.items()
                        if row["status"] == "done" and key not in uncertain}
    items: list[dict[str, Any]] = []

    def _reason_for_workstream(key: str, status: Any, unmet: list[str],
                               unavailable: list[str]) -> tuple[str, str, str]:
        if key in uncertain:
            return "unknown", "status_unknown", f"Authored readiness source for WS:{key} is ambiguous or invalid."
        if status == "done":
            return "done", "status_done", "Authored workstream status is done."
        if status == "killed":
            return "done", "status_done", "Authored workstream status is killed; it is terminal and not actionable."
        if status in {"active", "awaiting_ci", "awaiting_review"}:
            return "in_progress", "status_in_progress", f"Authored workstream status is {status}."
        if status in {"blocked", "parked"}:
            return "blocked", "workstream_blocked", f"Authored workstream status is {status}."
        if status == "proposed":
            if unavailable:
                return "unknown", "status_unknown", f"Declared dependency sources are unavailable: {', '.join(unavailable)}."
            if unmet:
                return "blocked", "unmet_dependencies", f"Waiting for declared dependencies: {', '.join(unmet)}."
    ...
    def _reason_for_wave(key: str, status: Any, parent_status: Any, unmet: list[str],
                         unavailable: list[str]) -> tuple[str, str, str]:
        if key in uncertain:
            return "unknown", "status_unknown", f"Authored readiness source for WS:{key} is ambiguous or invalid."
        ...
        if status == "todo":
            ...
            if parent_status in {"blocked", "parked", "killed"}:
                return "blocked", "workstream_blocked", f"Parent workstream status is {parent_status}."
            if parent_status == "done":
                return "unknown", "status_unknown", "Parent workstream is done while this wave remains todo."
    ...
    for row in records:
        ...
        ws_unavailable = sorted(dep for dep in ws_dependencies
                                if dep.split(":", 1)[1] not in by_key
                                or dep.split(":", 1)[1] in uncertain)
        graph_unmet = sorted(dep for dep in ws_dependencies
                             if dep.split(":", 1)[1] not in done_workstreams)
        ws_unmet = [] if row["status"] in {"done", "killed"} else graph_unmet
        state, reason_code, reason = _reason_for_workstream(key, row["status"], ws_unmet, ws_unavailable)
        items.append({"workstream": key, "wave": None, "state": state,
                      "reason_code": reason_code, "reason": reason,
                      "depends_on": ws_dependencies, "unmet_dependencies": ws_unmet,
                      "source": row["source"]})
        done_waves = {str(wave["id"]) for wave in row["wave_detail"] if wave["status"] == "done"}
        known_waves = {str(wave["id"]) for wave in row["wave_detail"]}
        for wave in row["wave_detail"]:
            local_dependencies = {f"WS:{key}#{dep}" for dep in wave["depends_on"]}
            dependencies = sorted(set(ws_dependencies) | local_dependencies)
            unavailable = sorted(set(ws_unavailable) | {ref for ref in local_dependencies
                                                         if ref.split("#", 1)[1] not in known_waves})
            unmet = sorted(set(ws_unmet) | {ref for ref in local_dependencies
                                           if ref.split("#", 1)[1] not in done_waves})
            if wave["status"] in {"done", "dropped"}:
                unmet = []
            state, reason_code, reason = _reason_for_wave(key, wave["status"], row["status"], unmet, unavailable)
            items.append({"workstream": key, "wave": wave["id"], "state": state,
                          "reason_code": reason_code, "reason": reason,
                          "depends_on": dependencies, "unmet_dependencies": unmet,
                          "source": row["source"]})
    items.sort(key=lambda item: (item["workstream"], item["wave"] is not None, item["wave"] or ""))
    return {"schema": READINESS_SCHEMA, "records": items, "degraded": sorted(set(degraded))}
...
def build_state(store: Store, *, now: _dt.datetime, degraded: Degraded,
                builds: dict[str, Any] | None, p0_status: dict[str, str] | None,
                worktrees: dict[str, Any]) -> dict[str, Any]:
    records, warnings = build_records(store, now=now, builds=builds,
    ...
    if builds:
        ...
        if moment:
            ...
            if age_hours > 36:
                degraded.add(f"active_builds.v1 is {age_hours:.0f}h old — PR state predates the last nightly sweep")
    return {
        "schema": STATE_SCHEMA, "generator": "scripts/agentos.py status",
        "generated_at": _iso_z(now),
        "inputs": {
            "workstreams": store.counts["workstreams"], "decisions": store.counts["decisions"],
            "discoveries": store.counts["discoveries"], "handoffs": store.counts["handoffs"],
            "active_builds": f"{_rel(_ACTIVE_BUILDS)}@{stamp}" if stamp else None,
            "active_builds_age_hours": age_hours, "worktrees": worktrees.get("count", 0),
            "uncommitted_worktrees": worktrees.get("uncommitted") or [], "degraded": degraded.items,
        },
        "workstreams": records,
        "needs_ceo": [{"workstream": row["key"], **row["needs_ceo"], "source": row["source"]}
                      for row in records if row["needs_ceo"]],
        "readiness": compute_readiness(records, degraded=readiness_issues,
...
def render_state_markdown(state: dict[str, Any]) -> str:
    ...
    lines += [f"Generated: {state['generated_at']}  |  {len(state['workstreams'])} workstreams ({rollup})",
              "", "| Input | Value |", "|---|---|",
              f"| active_builds | {inputs['active_builds'] or '_absent_'} |",
              f"| active_builds age | {age_cell} |", f"| worktrees | {inputs['worktrees']} |",
              f"| records | {inputs['workstreams']} WS · {inputs['decisions']} DEC · {inputs['discoveries']} DSC · {inputs['handoffs']} handoffs |", ""]
    if inputs["degraded"]:
        lines += ["## Degraded inputs", ""] + [f"- {item}" for item in inputs["degraded"]] + [""]
    lines += ["## Workstreams", "", "| Key | Status | Owner | Program | Waves | PRs | Next action |",
              "|---|---|---|---|---|---|---|"]
    for row in state["workstreams"]:
        waves = " ".join(f"{k}:{v}" for k, v in row["waves"].items() if v)
        prs = " ".join(f"#{p['number']}({p['state']})" for p in row["prs"]) or "—"
        lines.append(f"| [`WS:{row['key']}`]({_md_link(row['source'])}) | {row['status']} | {row['owner']} | {row['program']} | {waves or '—'} | {prs} | {_md_cell(row['next_action'])} |")
    lines.append("")
    if state["needs_ceo"]:
        lines += ["## Needs a CEO ruling", ""]
        for item in state["needs_ceo"]:
            when = f" · wanted by {item['by_when']}" if item["by_when"] else ""
            lines.append(f"- **WS:{item['workstream']}** — {_md_cell(item['question'])} (blocks {item['blocks_waves']} wave(s){when}) — `{item['source']}`")
        lines.append("")
    if state["warnings"]:
        lines += ["## Warnings", ""] + [f"- {_md_cell(w)}" for w in state["warnings"]] + [""]
    return "\n".join(lines)
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if now is None:
        print(f"::warning title=agentos::--now {args.now!r} unparseable — using wall clock", flush=True)
        now = _dt.datetime.now(_dt.timezone.utc)
    if not store_root.exists():
        print(f"::warning title=agentos::no agentos/ store at {store_root} — nothing to report; existing outputs left untouched", flush=True)
        return 0
    programs = _load_programs()
    store = load_store(store_root, programs)
    ...
    if hard:
        for problem in hard:
            ...
            store.records = {k: v for k, v in store.records.items() if store.paths.get(k) != problem.path}
    ...
    if args.dry_run:
        ...
        for target, text in ((Path(args.json_out) if args.json_out else _STATE_JSON, payload),
            ...
            try:
                ...
                print(f"::warning title=agentos::could not write {target} ({exc}) — existing output left untouched", flush=True)
    for item in degraded.items:
        print(f"::warning title=agentos-degraded::{item}", flush=True)
    print(f"agentos status: {len(state['workstreams'])} workstreams · {len(state['needs_ceo'])} needing a ruling · {len(state['readiness']['records'])} readiness record(s) · {len(state['warnings'])} warning(s) · {len(degraded.items)} degraded input(s)", flush=True)
    return 0
...
def _since_window(text: str | None, now: _dt.datetime) -> tuple[_dt.datetime, str]:
    if text:
        ...
        if match:
            ...
            delta = _dt.timedelta(hours=amount) if match.group(2) == "h" else _dt.timedelta(days=amount)
...
def build_brief(state: dict[str, Any], *, now: _dt.datetime, since: _dt.datetime,
                since_label: str) -> dict[str, Any]:
    ...
    for row in records:
        for wave in row["wave_detail"]:
            ...
            if moment is None or moment < since:
                continue
            finished.append({"workstream": row["key"], "wave": wave["id"],
                             "title": wave["title"], "prs": wave["prs"],
                             "done_at": wave["done_at"], "source": row["source"]})
    finished.sort(key=lambda f: (f["done_at"] or "", f["workstream"]), reverse=True)
    blocked = [{"workstream": row["key"], "title": row["title"],
                "blocked_by": row["blocked_by"], "record_stale_days": row["stale_days"],
                "source": row["source"]} for row in records if row["status"] == "blocked"]
    blocked.sort(key=lambda b: (-(b["record_stale_days"] or 0), b["workstream"]))
    needs = sorted(state["needs_ceo"], key=lambda n: (n["by_when"] or "9999-99-99",
                                                       -n["blocks_waves"], n["workstream"]))
    claims = [row for row in records if row["claim"]]
    running = {
        "active": by_status.get("active", 0), "awaiting_ci": by_status.get("awaiting_ci", 0),
        "awaiting_review": by_status.get("awaiting_review", 0), "blocked": by_status.get("blocked", 0),
        ...
        "claims_without_worktree": sum(1 for row in claims if not row["claim"]["stale"] and not row["claim"]["worktree_live"]),
    }
    return {
        "schema": BRIEF_SCHEMA, "generated_at": _iso_z(now), "since": _iso_z(since),
        "since_label": since_label,
        "counts": {"total": len(records), "active": by_status.get("active", 0),
                   "awaiting_ci": by_status.get("awaiting_ci", 0),
                   "blocked": by_status.get("blocked", 0), "done_in_window": len(finished)},
        "inputs": {"active_builds_age_hours": state["inputs"]["active_builds_age_hours"],
                   "worktrees": state["inputs"]["worktrees"], "degraded": state["inputs"]["degraded"]},
        "needs_ceo": needs, "blocked": blocked, "finished": finished, "running": running,
        "readiness": state["readiness"], "warnings": state["warnings"],
    }
...
def render_brief(brief: dict[str, Any], *, full: bool, state: dict[str, Any]) -> str:
    ...
    out.append(f"since {brief['since_label']} ({brief['since'][:16].replace('T', ' ')} UTC{hours})")
    out.append("")
    out.append(f"  {counts['total']} workstreams:  {counts['active']} active · {counts['awaiting_ci']} awaiting CI · {counts['blocked']} blocked · {counts['done_in_window']} done this window")
    ...
    for index, item in enumerate(brief["needs_ceo"][:CAP_NEEDS_CEO], start=1):
        blocks = f" — {item['blocks_waves']} unfinished wave(s)" if item["blocks_waves"] else ""
    ...
    if brief["finished"]:
        ...
        for item in brief["finished"][:cap]:
            ...
            out.append(f" ✅ WS:{item['workstream']} {item['wave']} — {_md_cell(item['title'] or '')}  {prs}")
        _overflow(out, len(brief["finished"]), cap, "finished")
        out.append("")
    running = brief["running"]
    out.append(f"━━ RUNNING (no action needed) {RULE[:39]}")
    out.append("")
    out.append(f" {running['active']} active · {running['awaiting_ci']} awaiting CI · {running['awaiting_review']} awaiting review · {running['proposed']} proposed.")
    out.append(f" {running['open_prs']} open PR(s) cited by a wave. {running['stale_claims']} stale claim(s); {running['claims_without_worktree']} claim(s) with no live worktree.")
    out.append("                                     → agentos.py brief --full")
    out.append("")
    if full:
        out.append(f"━━ FULL DETAIL {RULE[:53]}")
        out.append("")
        out.append("Claim notes are ADVISORY — an author's note in git, not live activity. Live worker/job state is the Executive OS runtime (control_plane/); occupancy evidence is `git worktree list`.")
        out.append("")
        for row in state["workstreams"]:
            claim = row["claim"]
            ...
            out.append(f" {row['key']:<28} {row['status']:<16} {row['owner']:<12} {claim_text}")
            out.extend(_wrap(f"next: {row['next_action']}", 60, "      "))
            for collision in row["collisions"]:
                out.extend(_wrap(f"collision with WS:{collision['with']} on {', '.join(collision['paths'])}", 60, "      ⚠ "))
        out.append("")
        if brief["warnings"]:
...
def cmd_brief(args: argparse.Namespace) -> int:
    ...
    if now is None:
        now = _dt.datetime.now(_dt.timezone.utc)
    if not store_root.exists():
        print(f"::warning title=agentos::no agentos/ store at {store_root} — no brief", flush=True)
        return 0
    ...
    builds = load_active_builds(degraded, Path(args.active_builds) if args.active_builds else None)
    state = build_state(store, now=now, degraded=degraded, builds=builds,
                        p0_status=load_p0(degraded), worktrees=scan_worktrees(degraded, deep=args.scan_uncommitted))
...
BUNDLE_BUDGET_DEFAULT = 8000
BUNDLE_BUDGET_MIN = 500
BUNDLE_CHARS_PER_TOKEN = 4
...
_INDEX_PROJECTS = {"macro": _MACRO_PROJECT, "terminal": "terminal", "mastermind": "mastermind"}
...
SECTIONS: tuple[tuple[str, str, str], ...] = (
    ("higher_law", "HIGHER LAW — outranks every Agent OS record below",
     "DNR rows, the company P0 objective, and the owning program row. Nothing in this bundle may be read as overriding these."),
    ("workstream", "WORKSTREAM STATE (authoritative for state, not for permission)",
     "What the work is and where it stands. Agent OS records work; it never decides whether work may run (invariant I1)."),
    ("decisions", "DECISIONS (institutional reasoning — do not override higher law)",
     "Current decisions only. Superseded records are named under EXCLUDED with the key that replaced them; they are never co-equal here."),
    ("discoveries", "DISCOVERIES (evidence, not policy)",
     "Fresh findings only. Expired, superseded, and uncited-and-stale discoveries are excluded by field, not by relevance."),
    ("handoff", "LATEST HANDOFF (transfer state, not strategy)",
     "Where the last session stopped. Only the latest survives — older handoffs are excluded rather than merged into a composite nobody wrote."),
    ("artifacts", "ARTIFACTS (pointers — read the primary source)",
     "Paths only. A record points at prose; it never absorbs it."),
)
PACK_ORDER = ("workstream_block", "higher_law", "handoff", "decisions", "discoveries", "dependency", "artifacts")
_PACK_SECTION = {"workstream_block": "workstream", "dependency": "workstream",
                 "higher_law": "higher_law", "handoff": "handoff", "decisions": "decisions",
                 "discoveries": "discoveries", "artifacts": "artifacts"}
...
def _bundle_item(*, kind: str, key: str | None, path: str, locator: str,
                 authority_class: str, status: str, updated: str | None,
                 excerpt: str, why: str) -> dict[str, Any]:
    return {"kind": kind, "key": key, "path": path, "locator": locator,
            "authority_class": authority_class, "status": status, "updated": updated,
            "excerpt": excerpt, "why_included": why}
...
def _pointer_authority(repo_key: str, path: str, degraded: Degraded) -> str:
    ...
    if spec is None:
        degraded.add(f"{_CONTEXT_INDEX_CONFIG_REL} has no project for repo {repo_key!r} — pointer authority defaulted to {POINTER_AUTHORITY_DEFAULT}")
        return POINTER_AUTHORITY_DEFAULT
...
def _supersession(rec: dict[str, Any], prefix: str,
                   known: dict[str, dict[str, Any]]) -> tuple[str | None, Any]:
    raw = rec.get("superseded_by")
    if raw is None:
        return None, None
...
def _default_search(query: str) -> dict[str, Any] | None:
    ...
    if not (db_dir / _MACRO_DB).exists():
        return None
    from engine.context_index.packet import build_packet
    return build_packet(query=query, db_dir=db_dir, project_db_map={_MACRO_PROJECT: _MACRO_DB},
                        repo_root_map={_MACRO_PROJECT: _ROOT}, mode="research",
                        max_results=SEARCH_MAX_RESULTS)


def _search_packet(query: str, search_fn: Callable[[str], dict[str, Any] | None] | None,
                   degraded: Degraded) -> dict[str, Any] | None:
    fn = search_fn or _default_search
    try:
        packet = fn(query)
    except Exception as exc:
        degraded.add(f"context index query failed ({exc.__class__.__name__}) — context index unavailable — pass --workstream")
        return None
    if not isinstance(packet, dict):
        return None
    if packet.get("index_stale"):
        degraded.add("context index is STALE (indexed sha != repo HEAD) — a record written since the last build is not retrievable; rebuild with scripts/context_index_build.py")
    if packet.get("no_answer_reason"):
        degraded.add(f"context index returned no answer: {packet['no_answer_reason']}")
    return packet


def _vote_for_paths(packet: dict[str, Any], store: Store) -> dict[str, int]:
    ws = store.of_type("WS")
    ...
    def owners(path: str) -> list[str]:
        ...
        for key in sorted(ws):
            ...
            for entry in rec.get("owns_paths") or []:
                if isinstance(entry, str):
                    resolved = _resolve_path(entry, rec)
                    if resolved and resolved[0] == "macro" and fnmatch.fnmatch(path, resolved[1]):
                        hits.add(key)
        return sorted(hits)
    votes: dict[str, int] = {}
...
def _candidate_rows(ws: dict[str, dict[str, Any]], keys: Iterable[str],
                    votes: dict[str, int], why: str) -> list[dict[str, Any]]:
    return [{"key": f"WS:{key}", "title": _flat(ws[key].get("title")),
             "score": votes.get(key, 0), "why": why} for key in keys]


def _resolve_target(store: Store, *, workstream: str | None, task: str | None,
                    search_fn: Callable[[str], dict[str, Any] | None] | None,
                    degraded: Degraded) -> dict[str, Any]:
    ws = store.of_type("WS")
    everything = _candidate_rows(ws, sorted(ws), {}, "no workstream could be resolved — every one is a candidate")
    if workstream is not None:
        key = _normalize_ws_key(workstream)
        if key in ws:
            return {"key": key, "resolution": "explicit", "candidates": [], "no_answer_reason": None}
    ...
    if not ws:
        return {"key": None, "resolution": "unresolved", "candidates": [],
                "no_answer_reason": "the record store holds no workstreams — there is nothing to compile context from"}
    text = task or ""
    cited = sorted({m for m in WS_MENTION_RE.findall(text) if m in ws})
    if len(cited) == 1:
        return {"key": cited[0], "resolution": "cited-key", "candidates": [], "no_answer_reason": None}
    if len(cited) > 1:
        named = ", ".join(f"WS:{k}" for k in cited)
        return {"key": None, "resolution": "ambiguous",
    ...
    if packet is None:
        ...
        return {"key": None, "resolution": "unresolved", "candidates": everything,
                "no_answer_reason": "context index unavailable — no workstream could be resolved from free text; pass --workstream"}
    votes = _vote_for_paths(packet, store)
    if not votes:
        return {"key": None, "resolution": "unresolved", "candidates": everything,
                "no_answer_reason": "no search result maps to a workstream — pass --workstream"}
    ranked = sorted(votes.items(), key=lambda item: (-item[1], item[0]))
    if len(ranked) == 1 or ranked[0][1] >= 2 * ranked[1][1]:
        return {"key": ranked[0][0], "resolution": "search", "candidates": [], "no_answer_reason": None}
    top, second = ranked[0], ranked[1]
    return {"key": None, "resolution": "ambiguous",
            "candidates": _candidate_rows(ws, [k for k, _ in ranked], votes, "ranked by context-index votes"),
            "no_answer_reason": f"WS:{top[0]} ({top[1]}) does not outrank WS:{second[0]} ({second[1]}) by 2x — pick one with --workstream"}
...
def _load_kill_rows(degraded: Degraded) -> dict[str, dict[str, Any]] | None:
    ...
    try:
        ...
        degraded.add(f"{_rel(_KILL_REGISTRY)} unreadable ({exc.__class__.__name__}) — DNR citations unresolved")
        return None
    entries = doc.get("entries") if isinstance(doc, dict) else None
    if not isinstance(entries, list):
        degraded.add(f"{_rel(_KILL_REGISTRY)} has no 'entries' list — DNR unresolved")
        return None
    return {str(row["key"]): row for row in entries if isinstance(row, dict) and isinstance(row.get("key"), str)}
...
def _load_program_row(program: str, degraded: Degraded) -> dict[str, Any] | None:
    ...
    try:
        ...
        degraded.add(f"{_PROGRAMS_REL} unreadable ({exc.__class__.__name__}) — program omitted")
        return None
    programs = doc.get("programs") if isinstance(doc, dict) else None
...
PROGRAM_EXCERPT_FIELDS = (("name", "name"), ("description", "description"),
                          ("purpose", "purpose"), ("strategic role", "strategic_role"),
                          ("lifecycle", "lifecycle_state"))


def _higher_law_items(rec: dict[str, Any], key: str, *, degraded: Degraded) -> list[dict[str, Any]]:
    items: list[dict[str, Any]] = []
    prose = list(rec.get("do_not_redo") or []) + list(rec.get("landmines") or [])
    cited = sorted({token for entry in prose if isinstance(entry, str) for token in DNR_RE.findall(entry)})
    rows = _load_kill_rows(degraded) if cited else None
    for token in cited:
        row = (rows or {}).get(token)
        if row is None:
            degraded.add(f"DNR:{token} cited by WS:{key} does not resolve in {_rel(_KILL_REGISTRY)} — cite it as DNR:<KEY>, never by row number")
            continue
        items.append(_bundle_item(kind="dnr", key=f"DNR:{token}", path=_DNR_SOURCE,
                                  locator=f"{_DNR_SOURCE}#{token}", authority_class="A1",
                                  status=str(row.get("verdict") or "unknown"), updated=None,
    ...
    if isinstance(p0, str) and p0:
        ...
        if p0_status is None:
            ...
            degraded.add(f"p0 {p0!r} on WS:{key} is not an objective in {label} — P0 context omitted")
        else:
            items.append(_bundle_item(kind="p0", key=None, path=label, locator=f"{label}#p0.{p0}",
                                      authority_class="A1", status=p0_status[p0], updated=None,
    ...
    if isinstance(program, str) and program:
        ...
        if row is not None:
            items.append(_bundle_item(kind="program", key=None, path=_PROGRAMS_REL,
                                      locator=f"{_PROGRAMS_REL}#programs.{program}",
                                      authority_class="A1", status=str(row.get("lifecycle_state") or "unknown"),
                                      updated=None, excerpt=_fields(row, PROGRAM_EXCERPT_FIELDS),
...
def _wave_line(wave: dict[str, Any], prs: dict[int, dict[str, Any]]) -> str:
    ...
    if wait:
        bits.append(f"wait: {wait['kind']} · review {wait['review_after']} · {wait['condition']}")
    if wave.get("next_action"):
        bits.append("next: " + _flat(wave["next_action"]))
    return " · ".join(bits)


def _workstream_excerpt(rec: dict[str, Any], prs: dict[int, dict[str, Any]]) -> str:
    lines = [f"objective: {_flat(rec.get('objective'))}"]
    lines.append(f"status: {rec.get('status')} · class: {rec.get('class')} · blast_radius: {rec.get('blast_radius')} · ambiguity: {rec.get('ambiguity')}")
    lines.append(f"owner: {rec.get('owner')} · program: {rec.get('program')} · repos: {_flat(rec.get('repos'))}")
    if rec.get("owns_paths"):
        lines.append(f"owns_paths: {_flat(rec.get('owns_paths'))}")
    wait = _wait_json(rec.get("wait"))
    if wait:
        lines.append(f"wait: {wait['kind']} · review {wait['review_after']} · {wait['condition']}")
    waves = [w for w in (rec.get("waves") or []) if isinstance(w, dict)]
    if waves:
        lines.append("waves:")
    ...
    claim = rec.get("claim")
    if isinstance(claim, dict):
        lines.append(f"claim (ADVISORY — an author's note in git, blocks nothing, proves no live activity): by {claim.get('by')}, expires {claim.get('expires')}")
...
def compile_bundle(store: Store, *, workstream: str | None = None, task: str | None = None,
                   now: _dt.datetime, budget: int = BUNDLE_BUDGET_DEFAULT,
                   builds: dict[str, Any] | None = None, degraded: Degraded | None = None,
                   search_fn: Callable[[str], dict[str, Any] | None] | None = None) -> dict[str, Any]:
    degraded = degraded if degraded is not None else Degraded()
    budget = max(BUNDLE_BUDGET_MIN, int(budget))
    target = _resolve_target(store, workstream=workstream, task=task,
                             search_fn=search_fn, degraded=degraded)
    envelope: dict[str, Any] = {
        "schema": BUNDLE_SCHEMA,
        "target": {"workstream": f"WS:{target['key']}" if target["key"] else None,
                   "task": task, "resolution": target["resolution"],
                   "candidates": target["candidates"], "wait": None},
        "generated_at": _iso_z(now), "repo_sha": _repo_sha(), "token_budget": budget,
        "token_estimate": 0, "sections": [], "excluded": [], "omitted_due_to_budget": [],
        "degraded": list(degraded.items), "no_answer_reason": target["no_answer_reason"],
    }
    ...
    rec = ws_all[key]
    envelope["target"]["wait"] = _wait_json(rec.get("wait"))
    ws_path = store.paths[f"WS/{key}"]
    program = rec.get("program") if isinstance(rec.get("program"), str) else None
    prs = _pr_index(builds)
    hard_paths = {problem.path for problem in store.problems
    ...
    def emit(pack: str, item: dict[str, Any]) -> None:
        item["_seq"] = order[0]; order[0] += 1; groups[pack].append(item)
    def drop(kind: str, ident: str | None, path: Path | str, reason: str) -> None:
        excluded.append({"kind": kind, "key": ident,
    ...
    emit("workstream_block", _bundle_item(kind="workstream", key=f"WS:{key}",
         path=_rel(ws_path), locator=f"{_rel(ws_path)}#frontmatter", authority_class="A4",
         status=str(rec.get("status") or "unknown"), updated=ws_updated,
         excerpt=_workstream_excerpt(rec, prs), why="the compilation target"))
    for field, label in (("landmines", "landmine"), ("do_not_redo", "settled — do not redo")):
        for entry in rec.get(field) or []:
            if isinstance(entry, str) and entry.strip():
                emit("workstream_block", _bundle_item(kind="constraint", key=f"WS:{key}",
                     path=_rel(ws_path), locator=f"{_rel(ws_path)}#{field}", authority_class="A4",
                     status=label, updated=ws_updated,
    ...
    for dep in sorted(_refs(rec.get("depends_on"), "WS")):
        ...
        if dep_rec is None:
            drop("dependency", f"WS:{dep}", "—", "dangling depends_on — no such record"); continue
        dep_path = store.paths[f"WS/{dep}"]
        if malformed("dependency", f"WS:{dep}", dep_path): continue
        _, dep_updated = git_dates(dep_path)
        emit("dependency", _bundle_item(kind="dependency", key=f"WS:{dep}", path=_rel(dep_path),
             locator=f"{_rel(dep_path)}#frontmatter", authority_class="A4",
             status=str(dep_rec.get("status") or "unknown"), updated=dep_updated,
             excerpt=f"{_flat(dep_rec.get('title'))}\nstatus: {dep_rec.get('status')}\nnext_action: {_flat(dep_rec.get('next_action'))}",
    ...
    for ref in sorted(cited_dec): reasons.setdefault(ref, []).append(f"cited by WS:{key}")
    for dec_key in sorted(dec_all):
        ...
        if f"WS:{key}" in affects: reasons.setdefault(dec_key, []).append(f"declares affects WS:{key}")
        if program and program in affects: reasons.setdefault(dec_key, []).append(f"declares affects program {program}")
        for entry in affects:
            hit = _owned_overlap(entry, dec_all[dec_key], owned)
            if hit is not None: reasons.setdefault(dec_key, []).append(f"affects paths this workstream owns ({entry} ~ {hit})")
    rows: list[dict[str, Any]] = []
    for dec_key in sorted(reasons):
        dec_rec = dec_all.get(dec_key)
        if dec_rec is None: drop("decision", f"DEC:{dec_key}", "—", "dangling citation — no such record"); continue
        dec_path = store.paths[f"DEC/{dec_key}"]
        if malformed("decision", f"DEC:{dec_key}", dec_path): continue
        replacement, raw = _supersession(dec_rec, "DEC", dec_all)
        if replacement is not None: drop("decision", f"DEC:{dec_key}", dec_path, f"superseded_by DEC:{replacement}"); continue
        if raw is not None: degraded.add(f"DEC:{dec_key} carries unresolvable superseded_by {raw!r} — retained; fix the record")
        supersedes = _refs(dec_rec.get("supersedes"), "DEC")
        note = "\n[supersedes " + ", ".join(f"DEC:{s}" for s in supersedes) + "]" if supersedes else ""
        body = _fields(dec_rec, (("question", "question"), ("answer", "answer"), ("rationale", "rationale")))
        _, dec_updated = git_dates(dec_path)
        rows.append({"_date": str(_as_date(dec_rec.get("decided_at")) or ""),
                     "item": _bundle_item(kind="decision", key=f"DEC:{dec_key}", path=_rel(dec_path),
                     locator=f"{_rel(dec_path)}#frontmatter", authority_class="A3",
                     status=str(dec_rec.get("confidence") or "unknown"), updated=dec_updated,
                     excerpt=_clip(body, EXCERPT_DECISION - len(note)) + note,
                     why="; ".join(reasons[dec_key]))})
    rows.sort(key=lambda row: row["item"]["key"]); rows.sort(key=lambda row: row["_date"], reverse=True)
    for row in rows: emit("decisions", row["item"])
    ...
    for ref in sorted(cited_dsc): dsc_reasons.setdefault(ref, []).append(f"cited by WS:{key}")
    for dsc_key in sorted(dsc_all):
        ...
        if key in scope or f"WS:{key}" in scope: dsc_reasons.setdefault(dsc_key, []).append(f"scoped to WS:{key}")
        if program and program in scope: dsc_reasons.setdefault(dsc_key, []).append(f"scoped to program {program}")
        for entry in scope:
            hit = _owned_overlap(entry, dsc_all[dsc_key], owned)
            if hit is not None: dsc_reasons.setdefault(dsc_key, []).append(f"scoped to paths this workstream owns ({entry} ~ {hit})")
    rows = []
    for dsc_key in sorted(dsc_reasons):
        dsc_rec = dsc_all.get(dsc_key)
        if dsc_rec is None: drop("discovery", f"DSC:{dsc_key}", "—", "dangling citation — no such record"); continue
        dsc_path = store.paths[f"DSC/{dsc_key}"]
        if malformed("discovery", f"DSC:{dsc_key}", dsc_path): continue
        replacement, raw = _supersession(dsc_rec, "DSC", dsc_all)
        if replacement is not None: drop("discovery", f"DSC:{dsc_key}", dsc_path, f"superseded_by DSC:{replacement}"); continue
        if raw is not None: degraded.add(f"DSC:{dsc_key} carries unresolvable superseded_by {raw!r} — retained; fix the record")
        ...
        if expires and expires < now.date():
            ...
            if dsc_key in cited_dsc: degraded.add(f"DSC:{dsc_key} cited by this workstream but expired {expires} — re-verify or supersede")
            continue
        ...
        if not citations.get(dsc_key, 0) and age is not None and age > STALE_DISCOVERY_DAYS:
            drop("discovery", f"DSC:{dsc_key}", dsc_path, f"uncited and {age}d old"); continue
        _, dsc_updated = git_dates(dsc_path)
        rows.append({"_date": str(verified or ""), "item": _bundle_item(kind="discovery",
            key=f"DSC:{dsc_key}", path=_rel(dsc_path), locator=f"{_rel(dsc_path)}#frontmatter",
            authority_class="A3", status=str(dsc_rec.get("confidence") or "unknown"),
            updated=dsc_updated, excerpt=_clip(_fields(dsc_rec, (("claim", "claim"),
            ("so what", "so_what"), ("falsifier", "falsifier"))), EXCERPT_DISCOVERY),
            why="; ".join(dsc_reasons[dsc_key]))})
    rows.sort(key=lambda row: row["item"]["key"]); rows.sort(key=lambda row: row["_date"], reverse=True)
    for row in rows: emit("discoveries", row["item"])
    mine = [stem for stem in sorted(hnd_all) if key in _refs(hnd_all[stem].get("workstream"), "WS")]
    def handoff_rank(stem: str) -> tuple[str, str]:
        match = HANDOFF_DATE_RE.search(stem); return (match.group(1) if match else "", stem)
    if mine:
        latest = max(mine, key=handoff_rank)
        for stem in mine:
            hnd_path = store.paths[f"HND/{stem}"]
            if stem != latest: drop("handoff", stem, hnd_path, f"older_handoff (latest: {latest})"); continue
            if malformed("handoff", stem, hnd_path): continue
            _, hnd_updated = git_dates(hnd_path)
            emit("handoff", _bundle_item(kind="handoff", key=f"WS:{key}", path=_rel(hnd_path),
                 locator=f"{_rel(hnd_path)}#frontmatter", authority_class="A4",
                 status=str(hnd_all[stem].get("ended_because") or "unknown"), updated=hnd_updated,
                 excerpt=_clip(_fields(hnd_all[stem], (("mission", "mission"),
                 ("state before", "state_before"), ("next actions", "next_actions"),
                 ("do not redo", "do_not_redo"), ("danger areas", "danger_areas"),
                 ("unresolved", "unresolved"))), EXCERPT_HANDOFF),
    ...
    for entry in rec.get("artifacts") or []:
        ...
        if resolved is None: degraded.add(f"artifact entry not a repo path — skipped: {entry.strip()}"); continue
        repo_key, rel = resolved; shown = rel if repo_key == "macro" else f"{repo_key}:{rel}"; root = roots.get(repo_key)
        if root is None: note = f"repo {repo_key!r} is not checked out — existence unknown"
        elif (root / rel).exists(): note = "present on disk"
        else: note = "NOT FOUND on disk (it may not exist yet)"
        emit("artifacts", _bundle_item(kind="artifact", key=None, path=shown, locator=shown,
             authority_class=_pointer_authority(repo_key, rel, degraded), status=note, updated=None,
             excerpt=f"pointer — {note}. Open the primary source; this bundle does not quote it.",
    ...
    selected: list[dict[str, Any]] = []; omitted: list[dict[str, Any]] = []; used = 0; always_cost = 0
    for rank, item in enumerate(ordered):
        cost = _item_cost(item)
        if id(item) in always or used + cost <= budget:
            selected.append(item); used += cost
            if id(item) in always: always_cost += cost
        else:
            omitted.append({"kind": item["kind"], "key": item["key"], "path": item["path"],
                            "tokens": cost, "rank": rank})
    chosen = {id(item) for item in selected}; by_section: dict[str, list[dict[str, Any]]] = {}
    for pack in PACK_ORDER:
        for item in groups[pack]:
            if id(item) in chosen: by_section.setdefault(_PACK_SECTION[pack], []).append(item)
    for items in by_section.values(): items.sort(key=lambda item: item["_seq"])
    for item in ordered: item.pop("_seq", None)
    tails = _estimate_tokens(json.dumps({"excluded": excluded, "omitted_due_to_budget": omitted,
                                         "degraded": list(degraded.items),
                                         "candidates": target["candidates"]}, ensure_ascii=False))
    used += tails
    if used > budget:
        degraded.add(f"token_estimate {used} exceeds token_budget {budget} — always-included constraint context ({always_cost}) + packed items ({used - tails - always_cost}) + accounting tails ({tails}); the always-included set and the tails are never dropped, so raise --budget")
    envelope["token_estimate"] = used
    envelope["sections"] = [{"id": ident, "title": title, "authority_note": note,
                              "items": by_section.get(ident, [])} for ident, title, note in SECTIONS]
    envelope["excluded"] = excluded; envelope["omitted_due_to_budget"] = omitted
...
def render_bundle(bundle: dict[str, Any]) -> str:
    ...
    overrun = " — OVER BUDGET, see DEGRADED" if bundle["token_estimate"] > bundle["token_budget"] else ""
    out: list[str] = [f"=== CONTEXT BUNDLE — {target['workstream'] or 'NO WORKSTREAM RESOLVED'} ===",
        f"task: {target['task'] or '—'}", f"resolution: {target['resolution']}",
        f"generated: {bundle['generated_at']}  ·  repo_sha: {bundle['repo_sha'][:12]}",
        f"budget: {bundle['token_budget']} tokens  ·  estimate: {bundle['token_estimate']} tokens{overrun}", ""]
    if bundle["no_answer_reason"]: out += _wrap(f"NO ANSWER: {bundle['no_answer_reason']}", 76, "") + [""]
    for candidate in target["candidates"]: out.append(f"  candidate {candidate['key']} (score {candidate['score']}) — {candidate['title']}")
    if target["candidates"]: out.append("")
    for section in bundle["sections"]:
        out.append(f"━━ {section['title']} {RULE[:max(0, 72 - len(section['title']))]}")
        out.extend(_wrap(section["authority_note"], 74, "   ")); out.append("")
        if not section["items"]: out += ["   (nothing in this section)", ""]; continue
        for item in section["items"]:
            ...
            for line in item["excerpt"].splitlines(): out.extend(_wrap(line, 70, "     ") or [""])
            out.append("")
    out.append(f"━━ EXCLUDED ({len(bundle['excluded'])}) {RULE[:56]}")
    for row in bundle["excluded"] or []: out.append(f" • {row['key'] or row['kind']} · {row['path']} — {row['reason']}")
    if not bundle["excluded"]: out.append(" (nothing excluded)")
    out.append(""); out.append(f"━━ OMITTED — BUDGET ({len(bundle['omitted_due_to_budget'])}) {RULE[:48]}")
    for row in bundle["omitted_due_to_budget"] or []: out.append(f" • {row['key'] or row['kind']} · {row['path']} — {row['tokens']} tokens (rank {row['rank']})")
    if not bundle["omitted_due_to_budget"]: out.append(" (nothing omitted — the bundle fits its budget)")
    out.append(""); out.append(f"━━ DEGRADED ({len(bundle['degraded'])}) {RULE[:56]}")
    for message in bundle["degraded"] or []:
        wrapped = _wrap(message, 70, "   "); out.append(" • " + wrapped[0].lstrip() if wrapped else " • "); out.extend(wrapped[1:])
    if not bundle["degraded"]: out.append(" (every input read cleanly)")
    out.append(""); return "\n".join(out)


def cmd_compile_context(args: argparse.Namespace) -> int:
    if bool(args.task) == bool(args.workstream):
        args._parser.error("give exactly one of a free-text task or --workstream <KEY>")
    store_root = Path(args.root) if args.root else _DEFAULT_STORE
    degraded = Degraded(); now = _parse_moment(args.now) if args.now else _dt.datetime.now(_dt.timezone.utc)
    if now is None:
        degraded.add(f"--now {args.now!r} unparseable — the wall clock was used instead"); now = _dt.datetime.now(_dt.timezone.utc)
    if not store_root.exists():
        if args.workstream:
            print(f"::error title=agentos-compile::no agentos/ store at {store_root} — cannot compile WS:{_normalize_ws_key(args.workstream)}", flush=True); return 1
        degraded.add(f"no agentos/ store at {store_root} — nothing to compile"); store = Store(store_root)
    else:
        store = load_store(store_root, _load_programs())
    if args.workstream:
        key = _normalize_ws_key(args.workstream); ident = f"WS/{key}"
        if ident not in store.records:
            print(f"::error title=agentos-compile::unknown workstream WS:{key} — no record at {store_root}/workstreams/WS-{key}.md", flush=True)
            for problem in store.problems:
                if problem.hard and problem.path.stem == f"WS-{key}": print(f"::error title=agentos-{problem.rule}::{problem.render(_ROOT)}", flush=True)
            return 1
        target_path = store.paths[ident]; mine = [p for p in store.problems if p.hard and p.path == target_path]
        fatal = [p for p in mine if p.rule not in CROSS_RECORD_RULES]
        if fatal:
            for problem in fatal: print(f"::error title=agentos-{problem.rule}::{problem.render(_ROOT)}", flush=True)
            print(f"::error title=agentos-compile::WS:{key} is malformed — fix the record before compiling context from it (fail-closed on schema, I4)", flush=True); return 1
        for problem in mine:
            if problem.rule in CROSS_RECORD_RULES: degraded.add(f"cross-record problem on WS:{key} — [{problem.rule}] {problem.message}; the bundle compiles around it")
    builds = load_active_builds(degraded, Path(args.active_builds) if args.active_builds else None)
    bundle = compile_bundle(store, workstream=args.workstream, task=args.task, now=now,
                            budget=args.budget, builds=builds, degraded=degraded)
    as_json = args.json or not args.text
    sys.stdout.write(_dump_json(bundle) if as_json else render_bundle(bundle)); return 0


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="agentos", description="Agent OS record validator and view generator.")
    ...
    p_status.add_argument("--scan-uncommitted", action="store_true", help="also scan every worktree for uncommitted source work")
    p_status.add_argument("--dry-run", action="store_true", help="print the JSON, write nothing")
    ...
    p_brief.add_argument("--scan-uncommitted", action="store_true", help="also scan every worktree for uncommitted source work")
    p_brief.add_argument("--no-remember", action="store_true", help="do not record this invocation as the last check-in")
    p_brief.set_defaults(func=cmd_brief)
    p_ctx = sub.add_parser("compile-context", help="bounded, cited context_bundle.v1 for a workstream or a task (Phase 3)")
    p_ctx.add_argument("task", nargs="?", help="free-text task; resolved via the context index")
    p_ctx.add_argument("--workstream", help="workstream key — KEY, WS-KEY and WS:KEY are all accepted")
    p_ctx.add_argument("--budget", type=int, default=BUNDLE_BUDGET_DEFAULT,
                       help=f"token budget (default {BUNDLE_BUDGET_DEFAULT}, floor {BUNDLE_BUDGET_MIN})")
    p_ctx.add_argument("--text", action="store_true", help="human-readable output")
    p_ctx.add_argument("--json", action="store_true", help="emit context_bundle.v1 (the default; accepted for symmetry)")
