...
def main() -> int:
    ...
    if not cal:
        log.error("::error title=finra-backfill::could not read the settlement calendar")
        return 1
    ...
    try:
        for i, d in enumerate(todo, 1):
            ...
            if df.empty:
                log.warning("::warning title=finra-backfill::settlement %s returned no rows", d)
                continue
            frames.append(df)
            got += 1
            if got % CHECKPOINT == 0:
    ...
    if combined.empty:
        log.error("::error title=finra-backfill::no settlements returned data")
        return 1
