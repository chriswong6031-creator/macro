...
def render_markdown(packets: list[dict], generated_at: str) -> str:
    ...
    lines.append(f"*Candidates awaiting decision: {len(packets)}*")
    lines.append("")
    lines.append(
...
def main() -> int:
    ...
    queue_json: dict = {
        ...
        "n_candidates": len(packets),
        "candidates": packets,
        "note": (
            "Cross-domain review queue. Ordered by candidate_type bin then "
            "created_at. RF-16: crowds_with is context only — no composite "
            "ranking. All metrics are display-only."
        ),
    }
    ...
    # Print summary
    print(f"Review queue: {len(packets)} candidate(s) awaiting decision")
    for p in packets:
        sw = p.get("search_width_at_scan")
        sw_str = f" [search_width={sw}]" if sw is not None else ""
        print(
            f"  - {p.get('candidate_id','?')} "
    ...
    if dry_run:
        print(f"\n[DRY-RUN] Generated {len(packets)} packet(s). Use --write to emit to disk.")
        return 0
