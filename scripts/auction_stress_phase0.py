"""Phase-0 validation: does a Treasury "auction stress" signal PREDICT forward
yields, or is it priced within the session?  (ChatGPT's "Auction Stress Engine".)

ChatGPT's thesis: a weak coupon auction (low bid-to-cover, low indirect/real-money
share, high primary-dealer takedown) should be FOLLOWED by higher yields / a cheaper
duration sector, and that read should feed Bonds/Macro/S&P/Gold/BTC/USD scores.

House discipline: validate BEFORE wiring anything into a score. The decisive question
is FORWARD predictive content. Auction results print at 1:01pm ET and the Treasury
market reacts in seconds, so the null is "fully priced intraday -> zero forward edge,
display-only." We test that null honestly.
...
Run: .venv/bin/python -m scripts.auction_stress_phase0
"""
...
AUCT_PARQUET = DATA / "auction_results.parquet"
YLD_PARQUET = DATA / "par_yields.parquet"
