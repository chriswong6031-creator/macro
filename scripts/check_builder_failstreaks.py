"""Consecutive-failure escalation for resilient brun() builders — daily.yml ENGINE job.
...
Design principles (house laws):
 - Lane-gated writes: ONLY --lane nightly may WRITE the ledger (nightly is the
   sole advancer of forward ledgers).  Other lanes (closing-bell, asia-close)
   read + report but skip the ledger write with a logged note.
 ...
 - Gap tolerance: a streak is preserved across runs whose last_run_utc is within
   ...
   continuous freeze).
 - Stale-escalation decay (nightly lane, non-empty rc set): if a ledger entry's
   ...
   30-day zombie alerts on renames, which is worse.  Guard: if the rc set is
   EMPTY (art dir missing or crashed before writing any .rc files), the ledger
   is NOT mutated and escalation from ledger-only state is suppressed — real
   streaks must not be wiped by a run where the band itself died.
...
Run IMMEDIATELY AFTER the 'regional + desk builders' step in daily.yml ENGINE job.
"""
...
def _emit_annotation(slug: str, streak: int, first_fail_utc: str, label: str, last_rc: int) -> None:
    ...
    body = (
        ...
        f"see data/ci/builder_failstreaks.json"
    )
...
def _dispatch_ops_alert(breached: list[dict[str, Any]], root: Path) -> None:
...
def process_band(
    ...
    Parameters
    ----------
    art_dir:      Band scratch dir with <slug>.rc and <slug>.log files.
    ledger_path:  Path to builder_failstreaks.json.
    lane:         Only 'nightly' may write the ledger.
    ...
    # Find all .rc files in the art_dir
    try:
        rc_files = sorted(art_dir.glob("*.rc"))
    except Exception as exc:  # noqa: BLE001
        log.warning("check_builder_failstreaks: cannot glob art_dir (%s) — skip", exc)
        rc_files = []

    # Stale-escalation decay guard: if the band produced NO rc files at all
    # (art dir missing / band crashed before writing anything), do NOT mutate
    # the ledger — real streaks must not be wiped by a dead band run.
    rc_slugs: set[str] = {rc_path.stem for rc_path in rc_files}
    if lane == _WRITE_LANE and not rc_slugs:
        log.warning(
    ...
    for rc_path in rc_files:
        ...
        label = _read_label(art_dir, slug)
    ...
    # Stale-escalation decay: on the nightly lane with a non-empty rc set, drop
    ...
    # above, so rc_slugs is non-empty here whenever lane == _WRITE_LANE.
    if lane == _WRITE_LANE and rc_slugs:
    ...
    # Write updated ledger — ONLY on nightly lane
    if lane == _WRITE_LANE:
        ...
        log.info(
            "check_builder_failstreaks: ledger write skipped (lane=%r, not %r) — "
            "report-only mode",
            lane, _WRITE_LANE,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--lane", default="",
        help="Pipeline lane. Only 'nightly' may write the ledger.",
    )
    parser.add_argument(
    ...
    try:
        ...
        if breached:
            _dispatch_ops_alert(breached, root)
        else:
