...
def _decode_changed_files(raw: str | None) -> list[str] | None:
    ...
    if not isinstance(parsed, list) or not all(
        isinstance(path, str) and (not path or _changed_path_is_safe(path))
        for path in parsed
    ):
        return None
...
def load_authoritative_plan(
    ...
    *,
    ...
    changed_files_sha256 = _document_string(document, "changed_files_sha256")
    if changed_files_sha256:
        changed_files_sha256 = _validated_hex(
            changed_files_sha256,
            label="authoritative changed-files sha",
            lengths=(64,),
        )
    changed_files_count = _document_int(document, "changed_files_count")
    if bool(changed_files_sha256) != bool(changed_files_count):
        raise ManifestError(
            "authoritative changed-files digest/count are inconsistent"
        )
    ...
    if changed_files_file is not None or changed_from is not None:
        state, artifact_paths = _read_changed_files_handle(changed_files_file)
        if state not in {"list", "null"}:
            raise ManifestError(
                "authoritative changed-files artifact is "
                f"{state}: {changed_files_file or '$CI_CHANGED_FILES_FILE (unset)'}"
            )
        resolved_paths = artifact_paths if state == "list" else None
        artifact_digest = changed_files_digest(resolved_paths)
        if (
            artifact_digest != changed_files_sha256
            or len(artifact_paths) != changed_files_count
        ):
            raise ManifestError(
                "authoritative changed-files artifact does not match its plan "
                f"(sha {artifact_digest or '(no list)'} / "
                f"{len(artifact_paths)} path(s), expected "
                f"{changed_files_sha256 or '(no list)'} / "
                f"{changed_files_count} path(s))"
            )
        changed_paths = tuple(artifact_paths) if artifact_paths else None
        ...
        if consuming_plan:
            plan = load_authoritative_plan(
                ...
                changed_files_file=(
                    ...
                    or os.environ.get("CI_CHANGED_FILES_FILE")
                ),
            )
        else:
