...
from engine.cycles import (cycle_state, mtf_snapshot, early_signals, ladder_state,
                           entry_quality, regime_state, _tf_state, rsi_divergence)
...
def main():
    ...
    for fi, f in enumerate(sorted(glob.glob("data/stocks/*.parquet"))):
        ...
        for i in range(WIN, len(close)-F2-1, STEP):
            ...
            # MONTHLY timeframe (not in mtf_snapshot) — needs a LONGER causal window
            # than WIN (600d≈28mo < the 40 bars _tf_state needs), so resample a
            # ~2000-day trailing slice (still no look-ahead).
            try:
                mslice = close.iloc[max(0, i-2000):i+1]
                mser = mslice.resample("ME").last().dropna()
                mst = _tf_state(mser) if len(mser) >= 40 else {}
            except Exception:
                mst = {}
            d, t3, w = mtf.get("D",{}), mtf.get("3D",{}), mtf.get("W",{})
            conf = int(_turning_up(d)) + int(_turning_up(t3)) + int(_turning_up(w)) + int(_turning_up(mst))
            ...
            (mo_split["monthly up"] if _turning_up(mst) else mo_split["monthly not"]).append(rec)
