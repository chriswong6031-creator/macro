...
- **Computed by:** `engine/holdings_signals.py` `fund_flow_decomposition` computes `flow_usd` / `selection_usd` / `total_usd` per fund-ticker from the implied price; `engine/etf_consensus.py` `_finish_decomposition` sums them per ticker, nulls all three when `n_funds_usd == 0`, and sets `usd_complete` from `n_funds_usd` against `n_funds_any`.
...
- **Computed by:** `collectors/holdings.py` `active_changes_dir` (sum-ratio basis, feeds `conviction_pp`) versus `engine/holdings_signals.py` `_scale_factor` (median basis, feeds the decomposition). `_scale_factor` itself falls back to the sum ratio below `config.yml` `flow_min_scale_n` continuing constituents and says which it used in `scale_basis`.
...
- **Computed by:** `engine/etf_consensus.py` `weight_trajectory` (snapshot selection via `_trajectory_snapshots` → `engine/holdings_signals._usable_snapshots`, row hygiene via `collectors/holdings._drop_non_equity`), attached to rows by `attach_trajectories`, capped by `config.yml` `sparkline_cap`.
