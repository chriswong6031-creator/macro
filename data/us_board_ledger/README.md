...
| 2026-06-30 | W-C options: `opt_skew`, `opt_skew_5d_chg`, `opt_ivspread_rel`, `opt_pin_risk`, `opt_wall_dist_up/down_pct`, `opt_doi_slope_5d`, `opt_vanna_relief` | 9.2–9.4% | W-C snapshot stores begin 2026-06-30 (skew/ivspread). `opt_vanna_relief`'s 5-session basis is resolved by CALENDAR, not by row position (GAP DISCIPLINE, `engine/options_stamp.py`): when the `polygon_gex` summary store holds no row AT the session 5 back, the basis is unmeasurable and the column is null rather than a wider change mislabelled "5d". Because every name reads the same store on the same schedule that null is date-WIDE, so the cross-sectional tercile additionally refuses to rank an `as_of` whose measurable names fall below `_TERCILE_MIN_COVERAGE` (50%) — measured over the committed store, 3 of 32 dates (2026-07-13 / 07-22 / 07-24) rank only 5 of 356–375 measurable names, where every other date ranks 99.7–100% |
