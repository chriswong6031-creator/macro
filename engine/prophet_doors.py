...
turnover, Foresight Desk stage (:data:`FEATURE_KEYS`, prereg §9 addendum 2026-08-04); Door W:
era / regime / archetype + the Signal Episode Atlas matching-episode posteriors
(:data:`FEATURE_KEYS_W`, prereg §10). They exist so the promotion read can test the relay and
matching-episode hypotheses on US forward data from day one.
...
both degrade to nulls on any failure. `tests/test_prophet_doors.py::TestFireInvariance` and
`::TestDoorWFireInvariance` pin that each door's fire set is identical with features on and off.
