...
sb_cohort_flow     : sum of chg5 over the cohort from the southbound holdings
                     store (CONTEXT ONLY — never a gate)
...
def _compute_inner(
    ...
    # Thrust event: crossed from < 0.30 to >= 0.60 within 10 sessions (IHM §2.3)
    thrust_fired = (
        cohesion_10d_ago < THRUST_LOW
    ...
    # Southbound cohort net flow context (sum of chg5 where available)
