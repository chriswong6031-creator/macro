...
* computes a content digest that is STABLE under wall-clock/identity churn — the
  digest excludes exactly the volatile generation fields
  (``generation_id``, ``built_at``, ``rendered_at``, ``content_sha256``) so an
  identical owner input reproduces an identical digest across two builds;
...
# and the wall-clock stamps that legitimately differ between two builds of the
# same owner input. Everything else is content.
VOLATILE_GENERATION_FIELDS = ("generation_id", "built_at", "rendered_at", "content_sha256")
