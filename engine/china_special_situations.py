...
def _regime_chip_for_date(date_str: str) -> dict | None:
    ...
    try:
        ...
        try:
            ...
            return None
        quad = str(row["quad"]) if not isinstance(row, pd.DataFrame) else str(row["quad"].iloc[0])
        quad_name = str(row["quad_name"]) if not isinstance(row, pd.DataFrame) else str(row["quad_name"].iloc[0])
        liquidity = str(row["liquidity"]) if not isinstance(row, pd.DataFrame) else str(row["liquidity"].iloc[0])
        cycle = str(row["cycle"]) if not isinstance(row, pd.DataFrame) else str(row["cycle"].iloc[0])
