...
contains the native body. Consumers continue to call the reader named by the owner in
`vocabulary.v1.json`.
...
- `lib/evidence_foundation.py` performs semantic checks that JSON Schema cannot,
  including owner clock binding, deterministic identity, automatic-relation limits,
  append/supersede corrections, and replay lookahead refusal.
...
3. **Every clock keeps its native name.** The seven classes are
   ...
   owner fields; a reference cannot silently relabel a timestamp. Each owner entry
...
5. **Independence has three axes.** Source independence, information novelty, and
   economic/mechanism independence are separately declared on every relation. Shared
   upstreams cannot be called independent merely because two projections differ.
...
8. **Corrections append.** Amendments, restatements, source corrections,
   ...
   object, and preserve replay. In-place mutation is forbidden.
9. **Replay is honest.** `historical_replay` carries typed cutoffs, code revision,
   input digest, and vintage state. A clock beyond its same-class cutoff is refused.
   `current_rule_recomputation` is a different mode and may not be described as replay.
